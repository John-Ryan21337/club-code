...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Completion audio is deliberately per-device and opt-in: whether a machine
  // should make noise depends on that machine's speakers and where the person
  // is sitting, not on a shared server profile. Both switches default off so
  // an upgrade never starts playing audio on its own. Speech is limited to two
  // fixed phrases (see `DesktopCompletionSpeechSynthesizeInputSchema`), so no
  // thread or project text is ever spoken.
