...
describe("provider compatibility matrix", () => {
  it("pins exact public CLI and protocol versions", () => {
    expect(approvedProviderCliVersion("codex")).toBe("0.152.1");
    ...
    expect(PROVIDER_COMPATIBILITY_MATRIX.providers.claude.agentSdkVersion).toBe("0.3.239");
  });

  it("maps only compatibility-managed npm packages", () => {
    expect(approvedVersionForNpmPackage("@openai/codex")).toBe("0.152.1");
