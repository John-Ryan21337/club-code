...
pinned in the compatibility matrix, and tested in Club Code source.
`;
...
export function selectRecoveryProjectDirectory(
  ...
  if (!selected) {
    throw new Error(
      "Provider update requires --project-dir (or the package manager's INIT_CWD) so recovery never opens a provider in the Club Code checkout by accident.",
    );
  }
...
export function assertRecoveryProjectOutsideRepo(repoRoot: string, projectDirectory: string): void {
  ...
  if (
    ...
    throw new Error(
      "Recovery project directory must be outside the Club Code checkout so fallback never opens a provider in Club Code by accident.",
    );
  }
}
...
export function providerUpdateCommand(
  ...
  if (observation.provider === "codex" && observation.installKind === "standalone") {
    throw new Error(
      "Codex standalone update has no exact-version interface, so Club Code will not automate it. Install the approved standalone release manually or use an exact-version npm/pnpm layout.",
    );
  }
  ...
  throw new Error(
    `${observation.provider} install type is not safely updatable by the detached helper. ` +
      "Use its package manager after Club Code publishes support for that install layout.",
  );
}
...
function shortcutRepairCommand(repoRoot: string): ReadonlyArray<string> | null {
  ...
  const script = NodePath.join(repoRoot, "scripts", "repair-club-code-shortcuts.ps1");
...
export function launchCommand(
  ...
  if (platform === "win32") {
    ...
    if (!existsSync(launcher)) throw new Error(`Club Code launcher is missing: ${launcher}.`);
    return [
      powershell,
      "-NoLogo",
      "-NoProfile",
...
async function waitForClubCodeHealth(launcherPid: number): Promise<void> {
  const deadline = Date.now() + 30_000;
  let stableSince: number | null = null;
  while (Date.now() < deadline) {
    if (!processIsAlive(launcherPid)) {
      throw new Error(`Club Code launcher process ${launcherPid} exited during relaunch.`);
    }
    stableSince ??= Date.now();
    if (Date.now() - stableSince >= 10_000) return;
    await new Promise<void>((resolve) => setTimeout(resolve, 1_000));
  }
  throw new Error(`Club Code launcher process ${launcherPid} did not remain healthy.`);
}
...
export function killallProvedComplete(output: string): boolean {
  return (
    output.includes("No Club Code client/server processes found.") ||
    /Club Code killall targeted \d+ process\(es\); 0 failed or remained alive\./u.test(output)
  );
}
...
async function runHelper(manifestPath: string): Promise<void> {
  ...
  if (realpathSync.native(manifest.repoRoot) !== expectedRepoRoot) {
    throw new Error("Provider update manifest names a different Club Code checkout.");
  }
  ...
  if (
    ...
    throw new Error(
      "Club Code source changed after conformity gates; update cancelled before shutdown.",
    );
  }
  ...
  try {
    ...
    if (!killallProvedComplete(shutdown.stdout)) {
      throw new Error(`Club Code shutdown did not prove completion. ${shutdown.stdout.trim()}`);
    }
  } catch (error) {
    const shutdownFailure = new Error(
      `Club Code shutdown did not complete cleanly; provider installation was not attempted: ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
    ...
    try {
      ...
      console.error(
        `[conformity] Shutdown-failure notice could not be displayed; do not launch a second Club Code instance until remaining processes are stopped. ${noticeError instanceof Error ? noticeError.message : String(noticeError)}`,
      );
    }
    process.exitCode = 1;
    return;
  ...
  if (updateFailure !== null) {
    console.error(
      `[conformity] Provider update failed; attempting to restore Club Code first: ${updateFailure instanceof Error ? updateFailure.message : String(updateFailure)}`,
    );
  }
  ...
  try {
    ...
    await waitForClubCodeHealth(launcherPid);
    console.log(
      `[conformity] Club Code relaunched through checkout-bound process ${launcherPid} and remained alive through the bounded health window.`,
    );
  } catch (error) {
    console.error(
      `[conformity] Club Code relaunch failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    try {
  ...
  if (updateFailure !== null) {
    try {
      ...
      console.error(
        `[conformity] Club Code restarted, but the update-failure notice could not open: ${noticeError instanceof Error ? noticeError.message : String(noticeError)}`,
      );
    }
    process.exitCode = 1;
  }
...
async function main(): Promise<void> {
  ...
  if (args.dryRun) {
    console.log(
      `Provider conformity update dry run passed for ${pendingUpdates.map(({ provider }) => provider).join(", ")}; Club Code remains running.`,
    );
    return;
  }
  if ((await gitOutput(repoRoot, ["status", "--porcelain"])) !== "") {
    throw new Error(
      "Provider update requires a clean Club Code checkout so the tested build is reproducible.",
    );
  }
  await runRepositoryGates(repoRoot);
  if ((await gitOutput(repoRoot, ["status", "--porcelain"])) !== "") {
