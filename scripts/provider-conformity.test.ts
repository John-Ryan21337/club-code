...
describe("provider conformity workflow", () => {
  ...
  it("uses INIT_CWD for recovery and refuses an ambiguous missing project", () => {
    ...
    ).toThrow("outside the Club Code checkout");
    expect(() =>
      assertRecoveryProjectOutsideRepo(
        NodePath.resolve("/club-code"),
        NodePath.resolve("/operator/project"),
  ...
  it("rejects managed installer versions paired with the wrong provider package", () => {
    const texts = new Map([
      ...
      [
        "apps/desktop/resources/managed-runtime/install-managed-provider-runtime.ps1",
        'Install-ProviderPackage -PackageName "@openai/codex" -Version "2.1.224"\nInstall-ProviderPackage -PackageName "@anthropic-ai/claude-code" -Version "0.154.0"',
      ],
    ]);
    expect(
      validateSourcePins({
  ...
  it("requires killall output to prove that no Club Code process survived", () => {
    expect(killallProvedComplete("No Club Code client/server processes found.")).toBe(true);
    expect(
      killallProvedComplete(
        "Club Code killall targeted 3 process(es); 0 failed or remained alive.",
      ),
    ).toBe(true);
    expect(
      killallProvedComplete(
        "Club Code killall targeted 3 process(es); 1 failed or remained alive.",
      ),
    ).toBe(false);
  });
});
