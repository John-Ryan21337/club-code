...
if ($Mode -ne "recovery") {
  ...
  $message = if ($Mode -eq "shutdown-failed") {
    "Club Code could not prove that every existing process stopped, so it did not update providers or launch a second instance.`n`nDetails: $LogPath"
  } else {
    "The provider CLI update failed, but Club Code restarted successfully.`n`nDetails: $LogPath"
  }
  [void][System.Windows.Forms.MessageBox]::Show(
    $message,
    "Club Code provider update",
...
$choice = [System.Windows.Forms.MessageBox]::Show(
  "Club Code could not relaunch. Open a provider in the saved project directory?$recoveryLogDetail`n`nYes: Claude Code`nNo: Codex`nCancel: do nothing",
  "Club Code recovery",
