...
async function macFallback(input: FallbackArgs): Promise<void> {
  ...
  if (!osascript) throw new Error("Club Code relaunch failed. Open Claude Code or Codex manually.");
  if (input.mode !== "recovery") {
    const action =
      input.mode === "shutdown-failed"
        ? "Club Code could not prove that every existing process stopped, so it did not update providers or launch a second instance."
        : "The provider CLI update failed, but Club Code restarted successfully.";
    const result = spawnSync(
      ...
      [
        ...
        "on run argv",
        "-e",
        `display alert "Club Code provider update" message (${JSON.stringify(`${action} Details: `)} & item 1 of argv) as warning`,
        "-e",
        "end run",
        input.logPath ?? "",
      ],
  ...
  const result = spawnSync(
    ...
    [
      ...
      "on run argv",
      "-e",
      'button returned of (display dialog ("Club Code could not relaunch. Open a provider in the saved project?" & item 1 of argv) buttons {"Cancel", "Codex", "Claude Code"} default button "Claude Code")',
      "-e",
      "end run",
      recoveryDetail,
    ],
...
async function linuxFallback(input: FallbackArgs): Promise<void> {
  ...
  if (!zenity && !kdialog) {
    throw new Error(
      `Club Code relaunch failed. Open Claude Code or Codex manually from ${input.projectDirectory}.`,
    );
  }
  if (input.mode !== "recovery") {
    const message =
      input.mode === "shutdown-failed"
        ? `Club Code could not prove that every existing process stopped, so it did not update providers or launch a second instance. Details: ${String(input.logPath)}`
        : `The provider CLI update failed, but Club Code restarted successfully. Details: ${String(input.logPath)}`;
    const notice = zenity
      ? spawnSync(zenity, ["--warning", "--title=Club Code provider update", `--text=${message}`], {
          cwd: input.projectDirectory,
          shell: false,
          encoding: "utf8",
        })
      : spawnSync(kdialog as string, ["--title", "Club Code provider update", "--sorry", message], {
          cwd: input.projectDirectory,
          shell: false,
          encoding: "utf8",
        });
  ...
  const recoveryText = input.logPath
    ? `Club Code could not relaunch. The provider update also failed; details: ${input.logPath}. Open a provider in the saved project?`
    : "Club Code could not relaunch. Open a provider in the saved project?";
  const result = zenity
    ? spawnSync(
        zenity,
        [
          "--list",
          "--title=Club Code recovery",
          `--text=${recoveryText}`,
          "--column=Provider",
          "Claude Code",
          "Codex",
  ...
  throw new Error(`Club Code relaunch failed. Run ${executable} from ${input.projectDirectory}.`);
}
