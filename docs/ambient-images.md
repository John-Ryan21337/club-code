...
- The delete route in this base slice checks a settings snapshot, then unlinks. A concurrent settings write can adopt the same id between those operations. The separate maintenance slice adds a settings-write lock around the check and unlink; apply it for atomic reference protection. The snapshot check alone does not provide that guarantee.
