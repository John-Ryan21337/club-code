...
| Providers            | Codex, Claude, OpenCode, and a separate **LM Studio Local** provider remain available. See the [compatibility matrix](../packages/shared/src/providerCompatibility.ts) for reviewed versions. Available local models appear in the main model picker.                                                                                                                                                                                                                                |
...
| YouTube queues       | Club Code ships no playlist content. Users can import a local `.txt` list. Reimporting the same filename replaces that browser's list; a new filename adds another choice. Non-embeddable or unavailable videos are skipped within the bounded queue pass.                                                                                                                                                                                                                           |
...
| Supervised browser   | A temporary sandboxed desktop browser. Codex and Claude threads have access by default to the shared page, with a saved per-thread disable option and one-time origin authorization. Hide and Minimize retain pages and logins in a bottom tab strip (up to eight tabs). Move or resize the floating panel, or use adjustable 50/50 chat/browser split view. End tab session clears that tab; app restart clears all tabs.                                                           |
...
Club Code does not include playlist files or preloaded media. Imported queue
libraries are local to that browser or device. Active queues remain session-only.
A same-name import replaces the local list. A distinct filename adds a selectable
list. An item that does not permit embedding is skipped. It does not stop the
remaining queue.
