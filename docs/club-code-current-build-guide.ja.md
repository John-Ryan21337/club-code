...
| Provider            | Codex、Claude、OpenCode と、独立した **LM Studio Local** を利用できます。対応バージョンは [互換性マトリクス](../packages/shared/src/providerCompatibility.ts) で確認できます。利用可能なローカルモデルはメインのモデル選択に表示されます。                                                                                                                                                                                           |
...
| Atmosphere          | optional 全画面 snow/rain/Matrix、日英 mix、2ch glyph、fixed/rainbow/stream別 rainbow/music reactive、shimmer、live-work word、Flat/Forward/Reverse/Warp/Walk Forward/Walk Reverse。bundled Hexagons preset は `SeaOfLightsInDarkstaticy` 一本だけ。fresh install は全部 off。勝手に開店しないよ🚪 Matrix Walk はランダムな位置から設定した距離だけ落下し、中心から外向きの wind も調整できる🌧️🌈                                    |
...
| YouTube list        | 同梱 playlist はゼロ。local `.txt` を import し、同じ filename なら置換、新しい filename なら追加。embed 不可や unavailable は bounded pass 内で skip。持ち込み曲だけで乾杯ね🥂                                                                                                                                                                                                                                                      |
...
| Supervised browser  | 一時的な隔離セッションを使うデスクトップブラウザーです。共有中のページはCodex・Claudeのスレッドから標準で利用でき、スレッドごとに無効化できます。オリジンの共有は一度だけ承認します。Hide・Minimizeでページとログインを下部のタブ列に保持します（最大8タブ）。フローティング表示は移動・サイズ変更ができ、チャットとブラウザーの50/50分割も調整できます。End tab sessionで対象タブを終了し、アプリ再起動ですべてのタブを終了します。 |
...
Club Code は playlist ファイルも preloaded media も同梱しません。ここは持ち込み制よ🍸
