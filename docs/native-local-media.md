...
The profiles also set equal minimum and maximum audio bitrates at their existing 96/128 kbps targets. A nominal bitrate alone let silence produce a small live response that Chromium did not finish initializing within the probe deadline. The explicit encoder bounds repaired that case without changing the deadline or adding audible data.
...
The real Windows check used an isolated Electron profile, a native VLC child and generated WAV data. It verified actual Ogg response bytes, muted and paused HTML audio decoding, wrong-owner read/release denial, token revocation, source-path privacy, child exit and launch-file cleanup. The initial 8 kHz input returned an empty body; the explicit output format repaired it. A second check found the silent-stream initialization defect described above. After both profile repairs, a clean run passed all 12 checks with the original silent fixture and unchanged decode deadline. No personal media, account or live application session was used. Native playback on macOS/Linux and the later renderer controls remain separate validation steps; unit tests cover their path and launch policy without claiming a live OS probe.
