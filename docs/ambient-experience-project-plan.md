...
- Each independently reviewable function ships in its own clean pull request.
  Forward features are not mixed into the exact `4c9907c5` extraction ledger,
  and unrelated features are not bundled into the same PR.
...
| Cross-provider follow-up steering | Landed - validation pending | Native steer only with current capability/turn proof; otherwise a per-thread waiting queue; implicit interrupt is prohibited                |
| Usage and Model Pacing            | Landed - validation pending | Every configured provider has a truthful state; Codex paid facts and guarded Claude experimental extra usage are polled; pacing is advisory |
...
| Provider usage truth and paid data | Default-off operator consent lists every configured provider with disabled/unavailable/auth/unsupported/no-data/stale states. Codex maps supported paid balance and spend-control facts. Claude uses an account-bound, subscription/version-gated, disposable no-prompt experimental usage query; it never touches active turns. OpenCode remains explicitly unsupported.                | Three typechecks and 118 targeted tests pass. Fresh full suites pass: contracts 155/155, server 1,383/1,383 with one skip, and web 1,322/1,322. An earlier order-dependent Codex shadow-home fixture anomaly passed 12/12 alone and the complete clean rerun. The audit covers account switches, version/auth gates, cooldown, live-event merging, fact staleness, strict decoding, finite-cap precedence, and all-provider states. |
...
activity links with Network/web, Database/query, and Build/compile inputs all
checked; Rainbow Extra remains selectable. Null-source custom
...
and Model Pacing remain off until operator consent; their prepared values are a
...
## Cafe Code upstream migration entry

| PR ID    | Review unit                                      | Base/dependencies                        | Upstream boundary and acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------- | ------------------------------------------------ | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| USAGE-01 | Provider-neutral usage truth and Codex paid data | Cafe Code `dev` plus settings foundation | Stable optional capability/paid-data/fact-time contracts, default-off operator consent, one row for every configured provider, explicit unsupported/no-data/stale states, Codex supported balance/spend normalization, per-instance server cooldown, renderer/tests, and neutral canon. Separate from the exact `4c9907c5` extraction and Matrix/media work. All target platforms render truthful unavailable/unsupported states where a provider binary or API is absent. |
| USAGE-02 | Experimental Claude subscription and extra-usage | USAGE-01                                 | Account-bound disposable no-prompt control query, `pro`/`max`/`team`/`enterprise` and Claude Code >=2.1.216 gates, strict allowlist decoder, live-event/keyed-window merge, source-specific staleness, transcript-behavior redaction, lifecycle/policy tests, and Claude-specific canon. Requires Anthropic product/auth review before upstream enablement; it may be deferred without blocking the supported USAGE-01/Codex surface.                                      |

These migration units do not alter an active provider Query and do not include
automatic pacing, agent scheduling, or quota-based interruption. Those behaviors
require separate PRs and their own lifecycle tests.
...
| P1-06 | **Landed - validation pending.** Matrix should convey real network/database/build activity, not decorative fake traffic. | Project safe provider-observed categories into bounded pulses; expose independently persisted Network/web, Database/query, and Build/compile checkboxes beneath the master switch; filter unchecked categories before canvas encoding; connect only same-category endpoints with the exact same reported item/tool identity; route at hex angles with repeated glyph-colored packets and continuously wrapping trails. | All three inputs default on for legacy compatibility and all unchecked yields no overlay; no agent/operation/dependency/temporal-only or cross-category edges; random or Matrix palette stays synchronized across routes/endpoints; use three repeated packets through ten links and two at the 11- and 12-link cap, never more than 30 packet instances per frame; marks use glyph-head opacity until a short terminal fade; 24-event/12-link/eight-second caps; reduced-motion static path; no prompts/URLs/SQL/IDs/paths/hidden traffic. |
...
| P2-07 | **Landed - validation pending.** Follow-ups must steer or wait without interrupting any configured provider.                                    | Use native live steering only from a current supported capability plus exact active-turn proof; otherwise retain thread-scoped intent until a normal safe boundary.                                                                                                    | All Claude-adapter models (including Opus, Sonnet, and Fable) and Codex make zero implicit Stop calls; unsupported, missing, stale, and changing capability states wait; the accessible waiting control is inert; only explicit Stop dispatches `thread.turn.interrupt`. |
