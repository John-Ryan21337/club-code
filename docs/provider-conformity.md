...
Club Code separates upstream discovery, source conformity, publication, and local CLI installation.
...
Run `corepack yarn providers:conform check` from a Club Code source checkout. It reports installed,
...
tests, the desktop build, and the offline conformity smoke before Club Code exits. It identifies
...
still current, consumes the manifest before any state-changing action, closes Club Code, installs
exact approved versions where the package manager supports them, verifies the binaries, repairs
recognized Windows Club Code shortcuts, relaunches, and proves that the checkout-bound launcher
...
verification, or partial-shutdown failure still attempts to restore Club Code first and then shows
an operator-visible notice pointing to the bounded log. If shutdown reports any surviving process,
the helper does not install providers or launch a second Club Code instance; it displays a dedicated
...
invocations may use `INIT_CWD`, but the resolved recovery project must exist outside the Club Code
checkout.
...
be proved. Club Code sets Claude Code's documented `DISABLE_AUTOUPDATER=1` only in provider processes
