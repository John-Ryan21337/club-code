...
The client seam returns `unknown`; this UI decodes the complete telemetry contract before reading
any field and rejects encoded, excess, contradictory, non-finite, stale, or far-future samples.
...
When **Hide unavailable metrics** is off, an unavailable metric remains as a text-only card and
never contains a graph. When the option is on, that entire card container is omitted and an empty
state remains when no measured metric exists. The option supports controlled state so an adopting
settings owner can persist it; this slice does not invent another settings store.
...
Raw transport errors and provider/platform detail strings are never rendered or forwarded. The
optional failure callback receives only one of the UI's fixed failure codes.

A client that ignores abort remains quarantined as the sole outstanding read. This preserves the
single-flight guarantee but necessarily pauses polling and project-switch admission until that
promise settles; no caller can guarantee both underlying single-flight and retry liveness when a
transport ignores cancellation forever. Production transport wiring must honor abort for liveness.
...
- persistence for the controlled **Hide unavailable metrics** preference.
