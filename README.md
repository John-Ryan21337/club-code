...
Listen, gorgeous, I wrote this down before the second bottle, so it is accurate. The new atmosphere and media effects start off. Their shared settings—and a validated uploaded ambient image/GIF—are saved private ambient content; **Local Media is the deliberate exception**, living only in the current document/session. The coding chat remains the main table; the sparkle never gets to steal your mouse, your prompt, or your tokens.

- **Snow, rain, or Matrix across the whole window:** Pick one, turn it off again, choose the color, transparency, speed, and bounded 0.50–2.50x density. It floats over the entire app but is pointer-transparent, motion-aware, bounded, and polite. Matrix can choose its Japanese-glyph ratio and optional reviewed enrichment, including rare intact cat-art tokens. Its color can stay fixed, make a smooth rainbow circuit, or react to a playing selected Local Media file. That last one reads only a tiny in-memory level from the approved local HTML-media analyser and falls back to the fixed/automatic color when the signal is quiet or unavailable—no microphone, system-audio capture, YouTube audio, or Spotify audio. Much cooler. Still not allowed to interrupt your typing.
- **Your image or GIF in the corner:** Upload a validated PNG, JPEG, WebP, or GIF; it is private stored ambient content, not a temporary Local Media selection. Start it in either lower corner, choose glow or no glow, and small, medium, large, or custom. Custom mode drags and resizes with the mouse or keyboard. If a video wants the same corner, the image stacks above it with a tidy 12 px gap; the image steps down in size or moves to the free side instead of wrestling in public. Replacing or removing an upload follows reference-aware cleanup rather than a reckless bulk purge. Animated GIF work pauses when reduced motion, visibility, or focus policy says it should.
- **YouTube without surrendering the room:** Paste a video or public-playlist URL, use the optional server-gated in-app public search, float the player in either corner, or enter Cinema mode. Cinema keeps the project rail on the left, gives the video the center, and moves chat into a right rail; the same player survives project, route, and layout changes. Native player fullscreen is still available. The player can have an optional bounded, pointer-pass-through glow with an accent or chosen color and intensity. For YouTube, **Match video** can derive four edge colors from the public thumbnail artwork for the currently trusted player video, giving the background a TV-light-like halo. It never reads cross-origin video frames, audio, cookies, or credentials; the small no-cookie artwork fetch is bounded, and an unavailable/unsupported palette safely keeps the regular fixed/accent glow. Spotify stays on that regular glow. On a configured local desktop backend, the owner can connect a YouTube account to browse owned playlists; the PKCE grant lives only in that owner session's memory and dies on disconnect, expiry, or restart. It is not remote-web OAuth, Premium login, or a promise that a private playlist can play in the iframe. Club Code never sees YouTube Premium credentials and does not extract YouTube audio for visualizers.
...
Atmosphere, image/GIF, streaming, Local Media, and opacity controls live under **Settings → Appearance** when the connected server and desktop expose the matching capability. Atmosphere includes bounded density (0.50–2.50x); Matrix also offers a Japanese glyph ratio, optional reviewed enrichment with rare intact cat-art tokens, and fixed/rainbow/music-reactive color modes. Music reactive is available only from the selected current-session Local Media element and safely falls back when it has no fresh signal. Workflow lives in the **Plan | Workflow** panel, where it can remain honest about unavailable provider detail.
...
- **雪・雨・Matrix 文字**をウィンドウ全部に降らせられるの。色も透明度も速さも、0.50–2.50x の上限つき density も選べて、もちろん off もある。Matrix は日本語 glyph の割合も、review 済みの enrichment も選べる。たまに cat-art token まで丸ごと落ちてくるの、ずるいでしょ。色は fixed、ゆっくり虹を一周する rainbow、再生中に選んだ Local Media の音で動く music-reactive。最後の子は approved な local HTML media analyser の小さな memory 内 level だけを見る。音が小さい・来てない時は fixed/auto color に戻るし、microphone、system audio、YouTube、Spotify の音には絶対手を出さない。クリックは奪わないし、動きすぎないよう上限もある。仕事の邪魔をせずに画面だけ急にかっこいい。ね、天才。もう一回見せて。
- **手元の画像/GIF**は、検査した PNG / JPEG / WebP / GIF を upload。これはその場だけの Local Media じゃなくて private に保存される ambient content。まず左下か右下。small / medium / large、それから mouse と keyboard で動かして resize できる custom。縁をふわっと光らせてもいい。同じ角に YouTube が来たら、その上へ 12 px だけ上品に空けて並ぶし、狭かったら画像だけ小さくなるか空いてる角へ行く。喧嘩しない。あたしたちより大人。replace/remove は参照を見てから片付けるから、まとめて勢いで消したりしない。GIF は reduced motion、非表示、focus、background animation の設定を守って休むから、目と CPU も朝まで働かされない。
- **YouTube**は URL や公開 playlist を入れて、左右に浮かべるか Cinema へ。Cinema では project sidebar が左、video が真ん中、chat が右。project や route を変えても player をむやみに作り直さないし、player 本来の fullscreen も使える。player のまわりは optional の、上限つき・pointer を通す CSS glow にできる。accent か好きな色、intensity も選べる。YouTube の **Match video** は、信頼できる今の player video の public thumbnail artwork から四方向の edge color を作って、TV の後ろの light みたいに背景へふわっと広げる子。でも cross-origin の video frame、音、cookie、credential は読まない。小さい no-cookie artwork fetch も上限つき、取れない・対応してない時は普通の fixed/accent glow のまま。Spotify はその普通の glow だけ。ここで勝手に覗き見しないの、えらい。server に API key を入れた時だけ、認証済みのアプリ内公開検索も出る。local desktop backend を設定した owner だけは自分の playlist を見るために account をつなげるけど、token はその owner session の memory だけ、restart/切断で消える。remote web OAuth でも Premium login でもない。Premium の password は受け取らないし、YouTube の音を抜いて visualizer にもしない。ここ大事。酔ってない字で書いといて。
...
本当に対応してる項目だけ出すの。Atmosphere の density は 0.50–2.50x の上限つき。Matrix は日本語 glyph の割合、review 済み enrichment、たまに丸ごとの cat-art token、それから fixed / rainbow / music-reactive の色も選べる。music-reactive は今の session で選んだ Local Media element だけ。fresh な signal がなければ安全に fallback。Workflow は `Plan | Workflow` の席にいる。きらきらは optional。仕事は main。順番、大事。
