...
- **Atmosphere depth that can walk:** Flat, Forward, Reverse, and Warp remain available. Walk Forward and Walk Reverse add a stronger near/far pass: falling Matrix glyphs default to a fluid `1.00px`-to-`72.00px` progression (or the exact reverse) before normal edge clipping/recycling. Both endpoints are editable from `0.10px` to `144.00px` at `0.10px` resolution; older hundredth-pixel preferences still decode and normalize safely. Snow and rain use the same absolute endpoint sizes, while activity routes, trails, packets, pulses, and telemetry use the same start-to-end perspective ratio with bounded decorative sizes. Warp routes narrow toward the center plane, and perspective routes flare toward the nearer endpoint without letting operational lines grow without bound.
- **Selected-project resource monitor:** A collapsible graph panel above the active chat shows host CPU and RAM, the selected project volume, aggregate host receive/transmit throughput, GPU, and VRAM. It polls only while expanded and visible, shares bounded host probes across projects, and never sends adapter names, addresses, endpoints, packet contents, or raw network counters to the renderer. Its panel and cards are fully transparent so the atmosphere remains visible; all six graph strokes derive from the current Matrix palette and follow its fixed, rainbow, extra/per-stream, music-reactive, shimmer, and reduced-motion frames.
...
- **YouTube without surrendering the room:** Paste a video or playlist, use optional server-side public search, or load a `.txt` queue containing one supported YouTube URL per line. Club Code ships the exact deduplicated operator-supplied [Japanese music list](./examples/youtube-url-queues/JPMusic.txt)—74 URL lines, of which the strict parser accepts 71 and visibly reports three malformed 10-character IDs—the [EDM list](./examples/youtube-url-queues/EDMYoutubeList.txt)—31 URL lines, 30 accepted and one malformed 10-character ID reported—and the [K-pop list](./examples/youtube-url-queues/KPOPList.txt)—eight accepted URL lines. All three are one-click choices in Settings. Importing a same-named `.txt` file replaces its local list, while a unique filename adds a new one-click choice; imported video IDs stay only in that browser/device library and are never uploaded or saved to server settings. In any untouched renderer session with no saved ambient-video source, the Japanese list initializes the session queue. Playback is requested only when the video surface is enabled; the branded fresh profile enables it, so its first accepted item requests autoplay. The source remains null and the active queue remains session-only. Browser policy, YouTube policy, embeddability, or an unavailable item may still prevent playback; videos that disallow embedding are skipped automatically. It never overwrites an existing ambient-video source, and a manual clear or queue/source choice wins for the rest of that session. Previous, next, manual skip, unavailable-item recovery, and playlist controls stay bounded. Floating mode supports presets and custom geometry; Cinema keeps projects on the left, gives video the middle, and moves chat into a right rail; native player fullscreen remains separate. A configured local owner may connect a YouTube account to discover playlists through memory-only PKCE. That is playlist discovery, not embedded Premium login, not password collection, and not a promise that a private or non-embeddable item will play.
...
- **LM Studio Local through Codex:** Settings > Providers always shows a distinct **LM Studio Local** setup row; **OpenCode** remains a separate provider. Club Code launches `codex --oss --local-provider lmstudio app-server`, skips cloud login checks, and discovers callable chat models from the configured OpenAI-compatible `/v1/models` endpoint without leaking Codex's cloud catalog into that instance. Modern LM Studio metadata is used to omit embedding-only models. The default is loopback (`http://127.0.0.1:1234/v1`), while literal private/LAN HTTP addresses such as `http://192.168.1.50:1234/v1` and HTTPS endpoints are also supported. Each provider instance receives its own process-scoped endpoint, so local and network LM Studio instances can coexist without changing global environment variables. Club Code does not automatically start LM Studio or load a model: start the LM Studio server, load a chat model (or enable Just-In-Time loading), then refresh provider status to make it selectable in the main chat model picker. HTTP hostnames are rejected because a syntax-only check cannot prevent DNS redirection; use a literal private IP or HTTPS. Embedded credentials, query strings, and non-API paths are also rejected. Important security limit: Codex's built-in `lmstudio` provider currently has no API-key or bearer-token hook, so it cannot connect when LM Studio's **Require Authentication** setting is on. LM Studio recommends authentication whenever it is served beyond loopback; until Codex exposes that hook, use this integration only on loopback or behind a trusted private network, VPN, or firewall, and never expose an unauthenticated endpoint to the public internet. Plain HTTP is unencrypted. HTTPS encrypts transport but does not authorize clients. LM Studio itself remains external and is not bundled.
...
For LM Studio, start its OpenAI-compatible API server, open Settings > Providers, and use the permanent **LM Studio Local** setup row. Loopback defaults to `http://127.0.0.1:1234/v1`; a trusted machine on the LAN can instead use its literal private IP, such as `http://192.168.1.50:1234/v1`. Club Code creates a separate Codex OSS instance for that endpoint; after refreshing provider status, its callable chat models appear in the main composer picker. Cloud login is not required for the LM Studio instance, embedding-only models are omitted, **OpenCode** remains separate, and an existing cloud Codex instance is left alone.
...
- **奥行きを歩く atmosphere**もあるの。Flat、Forward、Reverse、Warp はそのまま残して、Walk Forward と Walk Reverse を追加。Matrix の文字はデフォルトで `1.00px` から `72.00px` まで滑らかに大きくなるか、その逆で、端まで来たら普通に clip/recycle。開始・終了サイズはそれぞれ `0.10px` から `144.00px` まで `0.10px` 刻みで変更できて、昔の百分の一 pixel 設定も安全に読み込んで丸める。雪と雨は同じ絶対サイズを使い、activity route、trail、packet、pulse、telemetry は同じ開始・終了比率で奥行きを表しつつ、飾りの大きさは bounded のまま。Warp は中心へ細く潜って、perspective route は近い端だけふわっと太くなる。
- **選んだ project の resource monitor**は chat の上で畳める graph。host CPU/RAM、選択 project の volume、host 全体の受信/送信 rate、GPU、VRAM を、panel も card も完全透明で見せる。広げて画面が見える時だけ poll し、host probe は project 間で共有。adapter 名、address、endpoint、packet 中身、生 counter は renderer に渡さない。六本の線は今の Matrix palette から作るから、固定色、rainbow、stream ごとの Extra、音反応、shimmer、reduced motion まで同じ照明で踊る。
...
- **YouTube**は video、playlist、公開検索、それから URL を一行ずつ入れた `.txt` queue。重複を除いた operator-supplied の [Japanese music list](./examples/youtube-url-queues/JPMusic.txt) は 74 URL 行、そのうち strict parser が受けるのは 71、10 文字しかない malformed ID 三つはちゃんと report。[EDM list](./examples/youtube-url-queues/EDMYoutubeList.txt) は 31 URL 行、accepted 30、malformed な 10 文字 ID 一つを report。[K-pop list](./examples/youtube-url-queues/KPOPList.txt) は 8 URL 行すべて accepted。三つとも Settings から one-click で選べる。同じ filename の `.txt` を再 import すれば local list を置き換え、初めての filename なら新しい one-click option になる。import した video ID はその browser/device の library にだけ残り、server settings へ upload/save はしない。saved ambient-video source がない untouched renderer session なら Japanese session queue を初期化する。video surface が on の時だけ playback を request して、fresh profile はそこも on だから最初の accepted item に autoplay を request。source は null のまま、active queue は session-only。browser policy、YouTube policy、embed 不可、unavailable item なら再生されないことはあり、embed を許可しない動画は自動で skip する。既存の persisted ambient-video source は上書きしないし、いったんあなたが clear するか別の queue/source を選んだら、その session はあなたの指名が勝つ。前・次・skip があるし、再生不可なら bounded に次へ進む。floating、custom、Cinema、player 本来の fullscreen は別々。Cinema は project が左、video が中央、chat が右。account 接続は local owner が playlist を探すための memory-only PKCE で、Premium login でも password 預かりでもない。private/non-embeddable が急に再生できる魔法でもない。そこ盛ったら怒られるから、酔ってても盛らない。
...
LM Studio は OpenAI-compatible API server を起こして、Settings > Providers にいつも見える
**LM Studio Local** の setup row を使う。既定は `http://127.0.0.1:1234/v1`、信頼できる LAN の別マシンなら literal private IP も入力できる。endpoint ごとに別の Codex OSS instance ができて、status を refresh すれば使える chat model が main composer に並ぶ。embedding model は混ぜないし、**OpenCode** は別 provider のまま。LM Studio instance は cloud login いらないし、いつもの cloud Codex もそのまま。伝票わけた、はい乾杯。
