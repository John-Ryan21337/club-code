...
- **Atmosphere depth that can walk:** Flat, Forward, Reverse, and Warp remain available. Walk Forward and Walk Reverse add a stronger near/far pass: falling Matrix glyphs progress fluidly from `1.00px` through `72.00px` (or the exact reverse) before normal edge clipping/recycling, while snow, rain, activity packets, and bounded connector widths share the same depth cue. Warp routes narrow toward the center plane, and perspective routes flare toward the nearer endpoint without letting decorative operational lines grow without bound.
...
- **奥行きを歩く atmosphere**もあるの。Flat、Forward、Reverse、Warp はそのまま残して、Walk Forward と Walk Reverse を追加。Matrix の文字は `1.00px` から `72.00px` まで滑らかに大きくなるか、その逆で、端まで来たら普通に clip/recycle。雪、雨、packet、connector の太さも同じ奥行きに合わせるけど、飾り線は bounded のまま。Warp は中心へ細く潜って、perspective route は近い端だけふわっと太くなる。
