...
- **Keep-going controls without an immortal gremlin:** Auto Nudge is an explicit exact-thread preference with predefined plan-driven **Steady Progress** and **Hardcore Fanout** prompts; the prompt text is not currently editable per thread. Enabling it, mounting, reloading, or returning to an already-completed thread only establishes a baseline. A later changed provider-confirmed terminal identity grants one immediate foreground send after the complete safety gate is rechecked; no timer, countdown, elapsed-time cap, or wall-clock transition creates, delays, or repeats that authority. Queued operator work wins, and only the explicit Stop control may interrupt a turn. A separate default-off switch can persist one selected thread as the background owner with a 1–20 round cap plus visible pause/resume/stop state. At this PR7 head, however, the root coordinator cannot prove that owner's exact manual follow-up queue is empty, so it pauses before observation or transport. Background dispatch is therefore gated until durable exact-thread queue truth exists. Safe and a little boring beats waking up to a token bill with its own postcode.
...
5. **Keep the agent moving on your terms.** Foreground Auto Nudge waits for a newly observed exact terminal identity and never overtakes the selected thread's manual follow-up queue. Its per-thread mode and bounded background-owner state survive navigation, but background sending is intentionally paused at this head because the root coordinator does not yet have durable exact-thread queue truth. Usage and Model Pacing help decide when and how hard to continue. Ultra Caching and the Audit and Repair skill reduce avoidable recontextualization, while provider-reported cache counters and observed compaction remain separate measurable signals rather than an invented savings total.
...
- **Auto Nudge、でも不死身の妖怪はお断り。** mode は chat ごとの exact-thread 保存で、文面は plan-driven の **Steady Progress** と **Hardcore Fanout**。いまの head では prompt 本文を thread ごとに編集する機能はまだないよ。enable、mount、reload、昔の completed chat へ戻るだけでは送信権を作らず baseline にするだけ。あとで provider が確認した新しい terminal identity が来た時だけ、完全な safety gate を再確認して foreground の一回分を即時に許可する。timer、countdown、経過時間の上限、wall-clock transition は送信権を作らず、遅らせず、繰り返さない。manual queue が最優先で、interrupt は明示した Stop だけ。default-off の background owner と 1–20 rounds の上限、pause/resume/stop 状態は保存できる。でも PR7 の root coordinator は owner の exact manual FIFO が空だとまだ証明できないので、observe/transport より前に pause。background 送信は durable queue truth が入る lineage まで gated。請求書を朝のアフターに連れてこない、安全第一の子です🍸
...
5. **続けるかはあなたが決める。** foreground Auto Nudge は新しく観測した exact terminal identity を待ち、その thread の manual follow-up queue を追い越さない。per-thread mode と bounded background-owner state は Settings への寄り道でも残るけど、この head の background 送信は durable exact-thread queue truth がないので意図的に pause。Usage と Model Pacing で残りと reset を見て、Ultra Caching と Audit and Repair で recontext の無駄を減らす。cache reuse と observed compaction は別々に見て、架空の saved total にしない。気分で盛らない。ボトルの本数も token も会計は正直。
