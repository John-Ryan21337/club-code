...
>()("cafecode/desktop/ShellEnvironment") {}
...
const knownWindowsCliDirs = (env: NodeJS.ProcessEnv): ReadonlyArray<string> => [
  ...
  ...trimNonEmpty(env.USERPROFILE).pipe(
    Option.match({
      onNone: () => [],
      onSome: (value) => [`${value}\\scoop\\shims`],
    }),
  ),
];

const startMarker = (name: string) => `__CAFE_CODE_ENV_${name}_START__`;
const endMarker = (name: string) => `__CAFE_CODE_ENV_${name}_END__`;
