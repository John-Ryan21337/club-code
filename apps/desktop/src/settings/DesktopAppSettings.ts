...
import { fromLenientJson } from "@cafecode/shared/schemaJson";
...
const DesktopSettingsDocument = Schema.Struct({
  ...
  // preference recovers on its own without discarding unrelated desktop
...
function normalizeDesktopSettingsDocument(
  ...
  // A stored opacity outside the supported band, or of the wrong type, is
  // discarded together with the enable flag so recovery is fully opaque.
  const storedWindowOpacity =
    parsed.windowOpacity === undefined ? defaultSettings.windowOpacity : parsed.windowOpacity;
  const windowOpacityValid =
    typeof storedWindowOpacity === "number" &&
    Number.isFinite(storedWindowOpacity) &&
    storedWindowOpacity >= MIN_DESKTOP_WINDOW_OPACITY &&
    storedWindowOpacity <= MAX_DESKTOP_WINDOW_OPACITY;
  ...
  return {
    ...
    windowOpacity: windowOpacityValid ? storedWindowOpacity : defaultSettings.windowOpacity,
  };
}
