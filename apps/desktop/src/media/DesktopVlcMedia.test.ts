...
describe("DesktopVlcMedia", () => {
  it("drops provider secrets, loader hooks and user plugin paths from the decoder environment", () => {
    expect(
      DesktopVlcMedia.vlcChildEnvironment({
        SystemRoot: "C:\\Windows",
        HOME: "/synthetic",
        LANG: "ja_JP.UTF-8",
        ANTHROPIC_API_KEY: "synthetic-secret",
        OPENAI_API_KEY: "synthetic-secret",
        NODE_OPTIONS: "--require arbitrary",
        VLC_PLUGIN_PATH: "/arbitrary",
        PATH: "/arbitrary",
      }),
    ).toEqual({ SystemRoot: "C:\\Windows", HOME: "/synthetic", LANG: "ja_JP.UTF-8" });
  });

  it("bounds response streams and propagates downstream request cancellation", async () => {
    const harness = makeHarness();
    const signals: AbortSignal[] = [];
    vi.mocked(harness.dependencies.fetch).mockImplementation(async (_url, init) => {
      signals.push(init.signal!);
      return new Response(new ReadableStream(), { headers: { "Content-Type": "video/webm" } });
    });
    ...
    for (let index = 0; index < 4; index++) {
      responses.push(
        await Effect.runPromise(
          protocol.handler(
            new Request(
              selection!.playbackUrl,
              index === 0 ? { signal: cancellation.signal } : undefined,
            ),
          ),
        ),
      );
    ...
    expect(
      (await Effect.runPromise(protocol.handler(new Request(selection!.playbackUrl)))).status,
    ).toBe(429);
  ...
  it("retains admission for an unreaped child and never starts a third decoder", async () => {
    const harness = makeHarness();
    const first = await Effect.runPromise(harness.media.pick({ id: 42 }));
    harness.children[0]!.kill.mockImplementation(() => true);
    vi.useFakeTimers();
    try {
      const release = Effect.runPromise(
        harness.media.release({ id: 42 }, { sessionId: first!.sessionId }),
      );
      await vi.advanceTimersByTimeAsync(3_100);
      await release;
      await Effect.runPromise(harness.media.pick({ id: 43 }));
      await expect(Effect.runPromise(harness.media.pick({ id: 44 }))).rejects.toThrow();
  ...
  it.each(["wav", "flv"])(
    "uses a compatible Vorbis format for %s sources with low sample rates",
    async (extension) => {
      const harness = makeHarness({ filePath: mediaPath("Audio", `low-rate.${extension}`) });
