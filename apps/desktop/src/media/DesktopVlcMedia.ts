...
const MAX_ACTIVE_VLC_CHILDREN = 2;
const MAX_ITEM_RESPONSE_STREAMS = 4;
const noop = (): void => undefined;

/** Decoders do not need provider secrets, runtime hooks, or a user plugin path. */
export function vlcChildEnvironment(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
...
function makeSout(kind: DesktopLocalMediaKind, port: number, token: string): string {
  // VLC 3.0.12's managed Vorbis encoder can reject fixed bitrates at low input
...
export function make(
  ...
  const startItem = async (
    ...
    // Check after awaited preparation and immediately before synchronous spawn.
    // A child that ignored termination still occupies its slot in activeItems.
    if (activeItems.size >= MAX_ACTIVE_VLC_CHILDREN) {
      await removePrivateFilesWithRetry(privateLaunchPaths);
      throw new DesktopVlcMediaError({ reason: "VLC is busy stopping earlier playback." });
    }
    let child: VlcChild;
    try {
      child = dependencies.spawn(
        ...
        makeVlcArgs(dependencies.platform, configPath, playlistPath),
        {
          ...
          windowsHide: true,
          detached: false,
          env: vlcChildEnvironment(dependencies.env),
  ...
  const handleProtocolRequest = async (request: Request): Promise<Response> => {
    ...
    if (request.signal.aborted || item.requestControllers.size >= MAX_ITEM_RESPONSE_STREAMS) {
      return new Response(null, {
        status: request.signal.aborted ? 499 : 429,
        headers: { "Cache-Control": "no-store" },
      });
    }
