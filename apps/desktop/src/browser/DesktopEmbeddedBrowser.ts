...
export function makeDesktopEmbeddedBrowser(
  ...
  const open = async (
    ...
    const view = platform.createView(`cafe-code-embedded-${id}`);
  ...
  const targetPoint = async (
    ...
    // Electron drops native input when the owning window cannot receive it.
    if (!tab.ownerWindow.isFocused()) return null;
  ...
  const type = async (
    ...
    const approved = await approveSharedAction(
      tab,
      {
        ...
        detail:
          ...
          `at ${snapshotState(tab).displayUrl}? The value is not shown in this prompt or retained by Cafe Code.`,
    ...
    try {
      await tab.view.webContents.insertText(input.value);
    } catch {
