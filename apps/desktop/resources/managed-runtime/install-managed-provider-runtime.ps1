...
try {
  ...
  $results = @(
    Install-ProviderPackage -Name "Codex" -PackageName "@openai/codex" -Version "0.154.0" -ProviderSlug "codex" -BinaryName "codex" -ManagedRoot $managedRoot -NodeTarget $nodeTarget -NpmPath $npmPath -NpmCache $npmCache -NpmUserConfig $npmUserConfig
