...
export interface AmbientMediaGeometryClampOptions {
  /**
   * The media width divided by its height. Supply this together with
   * `paneAspectRatio` when the renderer knows both aspect ratios.
   */
  readonly mediaAspectRatio?: number;
  /**
   * The message-pane width divided by its height. Height remains absent from
   * persisted geometry and is derived from the final clamped width.
   */
 ...
 * without ever adding device geometry to backend-authoritative ClientSettings.
 */
...
/**
 * Converts finite geometry into a reachable normalized rectangle. The caller
 * may provide media and pane aspect ratios plus product size limits. Height is
 * then derived from the final clamped width, so width changes cannot leave a
 * panel unreachable. With no aspect ratios, this applies persistence-safe
 * bounds that do not require viewport information.
 */
...
/**
 * Decodes both the current envelope and the legacy unwrapped slot map. Invalid
 * slot values are reset independently so corruption in one media panel never
 * sacrifices usable geometry for the other panel.
 */
export function migrateAmbientMediaGeometryDocument(
  ...
  try {
    ...
    // The current format did not decode; try the only supported legacy shape.
  }
...
/**
 * Reads through the migration and recovery boundary. Rewriting a repaired
 * document is best-effort: even quota or privacy-mode failures must not make
 * otherwise usable geometry unavailable for the current renderer session.
 */
...
/**
 * Supports first entry into shared custom mode without coupling the store to
 * preset math. Existing usable local geometry wins; otherwise the caller's
 * currently resolved preset geometry is normalized and persisted. A storage
 * access or persistence failure returns `null`; callers can safely retain their
 * resolved preset presentation without claiming that custom geometry was
 * stored.
 */
