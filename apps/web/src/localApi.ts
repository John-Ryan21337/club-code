  ...
  return {
    ...
    server: {
      ...
      checkProviderAccess: (input) =>
        rpcClient
          ? rpcClient.server.checkProviderAccess(input)
          : Promise.reject(unavailableLocalBackendError()),
      interpretAtmosphereCommand: (input) =>
        rpcClient
          ? rpcClient.server.interpretAtmosphereCommand(input)
          : Promise.reject(unavailableLocalBackendError()),
      ...
      restartProviderRuntime: (input) =>
        rpcClient
          ? rpcClient.server.restartProviderRuntime(input)
          : Promise.reject(unavailableLocalBackendError()),
      openSystemPromptFile: () =>
        rpcClient
          ? rpcClient.server.openSystemPromptFile()
          : Promise.reject(unavailableLocalBackendError()),
      ...
      getRuntimeLayerDiagnostics: (input = {}) =>
        rpcClient
          ? rpcClient.server.getRuntimeLayerDiagnostics(input)
          : Promise.reject(unavailableLocalBackendError()),
      signalProcess: (input) =>
        rpcClient
