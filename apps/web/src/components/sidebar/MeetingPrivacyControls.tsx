...
/**
 * Sidebar entry point for meeting privacy: a direct on/off toggle plus a
 * manager dialog.
 *
 * The hidden project names and paths are the sensitive part of this feature,
 * so they are rendered only while the operator has the manager dialog open.
 * The dialog body is mounted on demand for that reason; keeping it mounted
 * would put the hidden names in the document during a screen share.
 */
        ...
        <TooltipTrigger
          render={
            <button
              ...
              data-testid="sidebar-meeting-privacy-toggle"
        ...
        <TooltipTrigger
          render={
            <button
              ...
              data-testid="sidebar-meeting-privacy-manage"
        ...
        {managerOpen ? (
          <DialogPopup className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Active provider turns continue in hidden folders. Saved follow-ups can wait until a
                chat is open. This device removes selected folders and their threads from the
                visible lists while meeting privacy is on. This setting does not change permissions
                or access. Existing system notifications and web push notifications are not hidden.
              </DialogDescription>
            </DialogHeader>
            <DialogPanel className="space-y-3">
              {hiddenProjects.length > 0 ? (
                ...
                <p className="rounded-md border border-dashed px-3 py-4 text-sm text-muted-foreground">
                  No project folders are selected. To select a folder, right-click a project and
                  select “Hide during meetings”.
                </p>
              )}
              {hiddenProjectKeys.length > hiddenProjects.length ? (
                <p className="text-xs text-muted-foreground">
                  The hidden list also holds folders from environments that are not connected now.
                  To show those folders again when they reconnect, select “Show all”.
                </p>
              ) : null}
            </DialogPanel>
            <DialogFooter>
