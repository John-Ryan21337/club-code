...
function aspectRatio(kind: LocalMediaKind): number {
  // Audio still needs room for its controls and toolbar.
...
export function LocalMediaPanel({
  ...
  const [resumeRequest, setResumeRequest] = useState<{ url: string; revision: number } | null>(
    null,
  );
  useEffect(() => {
  ...
  const handlePlaybackEnded = useCallback(async () => {
    const previous = localMediaStore.getSnapshot().source;
    const revision = localMediaStore.getSelectionRevision();
    const mayContinue = previous !== null && playedSourceRef.current === previous.objectUrl;
    playedSourceRef.current = null;
    const advanced = await localMediaStore.handlePlaybackEnded();
    const next = localMediaStore.getSnapshot().source;
    if (
      advanced &&
      mayContinue &&
      next &&
      mountedRef.current &&
      localMediaStore.getSelectionRevision() === revision + 1
    ) {
      setResumeRequest({ url: next.objectUrl, revision: revision + 1 });
    }
  }, []);
  ...
  useEffect(() => {
    if (
      !resumeRequest ||
      !mediaElement ||
      mediaElement.getAttribute("src") !== resumeRequest.url ||
      source?.objectUrl !== resumeRequest.url
    )
      return;
    setResumeRequest(null);
    if (localMediaStore.getSelectionRevision() !== resumeRequest.revision) return;
    // One continuation attempt after a successfully played item ends. Never
    // replay a late result after a new selection, Clear or workspace teardown.
    // Browser policy or a source transition can reject this attempt. Keep the
    // element and its manual Play controls; decode failures use onError below.
    void mediaElement.play().catch(() => undefined);
  }, [mediaElement, resumeRequest, source?.objectUrl]);
  ...
  const navigateQueue = useCallback(async (direction: "previous" | "next") => {
    ...
    const pending = localMediaStore.navigate(direction);
    const revision = localMediaStore.getSelectionRevision();
    const advanced = await pending;
    if (!advanced && mountedRef.current && localMediaStore.getSelectionRevision() === revision)
      setPlaybackError(true);
  }, []);
  const handlePlaybackFailure = useCallback(async () => {
    setPlaybackError(false);
    const pending = localMediaStore.handlePlaybackFailure();
    const revision = localMediaStore.getSelectionRevision();
    const advanced = await pending;
    // A late native navigation failure belongs to its original queue. It must
    // not remove the playback controls from a newer selection.
    if (!advanced && mountedRef.current && localMediaStore.getSelectionRevision() === revision)
      setPlaybackError(true);
  }, []);
  ...
  if (!source || (!cinemaEffective && !backgroundEffective && floatingAnchor === null)) return null;
  return (
    <section
    ...
    >
      ...
      >
        <h2
          ...
          className="min-w-0 flex-1 truncate rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
          Local media · {source.displayTitle}
          {state.queue ? ` · ${state.queue.currentIndex + 1}/${state.queue.totalItems}` : ""} ·
          session only
        </h2>
        ...
        >
          <video
            ...
            className={cn(
              ...
              backgroundEffective ? "object-cover" : "object-contain",
            )}
            ...
            onPlaying={() => {
              playedSourceRef.current = source.objectUrl;
              localMediaStore.markPlaybackSuccess();
            }}
            preload="metadata"
