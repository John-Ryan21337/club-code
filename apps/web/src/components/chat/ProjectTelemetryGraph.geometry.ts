...
/** A fresh panel starts at the right edge of its own chat anchor. */
export function defaultProjectTelemetryPanelGeometry(
  bounds: ProjectTelemetryPanelBounds,
): ProjectTelemetryPanelGeometry {
  return clampProjectTelemetryPanelGeometry(
    { ...PROJECT_TELEMETRY_PANEL_DEFAULT_GEOMETRY, x: bounds.width },
    bounds,
  );
}
