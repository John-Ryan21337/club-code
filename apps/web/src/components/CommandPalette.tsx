  ...
  const handleAddProject = useCallback(
    async (rawCwd: string) => {
      ...
      const existing = findProjectByPath(
        // Match against the full set. A hidden duplicate must be detected here,
        // otherwise the create path below would add a second project row for a
        // folder the operator chose to hide.
      ...
      if (existing) {
        ...
        if (!existingIsVisible) {
          // Do not name the folder here. The operator typed the path, so the
          // path is already on screen, but the project title is not.
          toastManager.add({
            type: "warning",
            title: "Project hidden for meeting privacy",
            description: "To show this project folder, open the meeting privacy manager.",
          });
          setOpen(false);
          return;
        }
