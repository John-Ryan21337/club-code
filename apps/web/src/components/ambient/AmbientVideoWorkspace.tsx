...
import { getDesktopTitlebarInset } from "../../lib/windowControlsOverlay";
...
export function AmbientVideoWorkspace({
  ...
  const capabilityEnabled = source !== null;
  ...
  const titlebarInset = getDesktopTitlebarInset();
  ...
  const retainMountedPlayer =
    ...
    rootRect.width >=
      (spotifySource
        ? FLOATING_WIDE_PANE_WIDTH
        : YOUTUBE_MINIMUM_VIEWPORT_HEIGHT + PLAYER_FRAME_BORDER_SIZE) &&
    rootRect.height >=
      YOUTUBE_MINIMUM_VIEWPORT_HEIGHT + MOBILE_PLAYER_CONTROLS_HEIGHT + PLAYER_FRAME_BORDER_SIZE &&
    (anchorRect === null || floatingVisible || streamingCinemaEffective) &&
    settings.ambientVideoEnabled &&
    capabilityEnabled &&
    sourceKey !== null &&
  ...
  const frameStyle = useMemo<CSSProperties>(() => {
    if (streamingCinemaEffective) {
      return {
        maxWidth: rootRect
          ? Math.max(356, (rootRect.height - titlebarInset - 64) * VIDEO_ASPECT_RATIO)
          : undefined,
      };
    }
