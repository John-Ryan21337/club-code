 ...
 * behavior. Ambient media is deliberately profile-owned: video sources are bounded opaque
 * identifiers and image references are authenticated, content-addressed server assets rather
 * than local paths or bytes.
 *
...
export const SETTINGS_PROFILE_CLIENT_FIELD_POLICY = {
  ...
  // Loading a profile is the explicit operator action that authorizes its
  // saved ambient-media state. Sources remain bounded by the shared schema and
  // do not contain provider credentials, local paths, or media bytes.
