...
describe("Matrix provider activity overlay", () => {
  ...
  it("uses exact bounded operation labels for verified telemetry rings without claiming rate", () => {
    expect(resolveMatrixActivityTelemetryLabel("network", "english")).toBe("FETCH • REPORTED •");
    expect(resolveMatrixActivityTelemetryLabel("database", "japanese")).toBe("照会 • 報告 •");
    expect(resolveMatrixActivityTelemetryLabel("build", null)).toBe("COMPILE • REPORTED •");
    expect(resolveMatrixActivityTelemetryLabel("agent", null)).toBe("DISPATCH • REPORTED •");
