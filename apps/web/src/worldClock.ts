...
/**
 * The selectable city catalog.
 *
 * Each entry binds one contract location id to an explicit IANA time zone, so
 * daylight-saving transitions come from the platform time-zone database rather
 * than a stored offset. The coordinates exist only so the optional weather
 * request can name a fixed point; they are constants in this file and never
 * come from geolocation or from any provider payload.
 */
...
// `Intl.DateTimeFormat` construction is expensive relative to `format()`, and
// the panel formats every visible city once per second. The cache keeps that
// tick allocation-light and independent of how long the panel stays open.
...
/**
 * Split one instant into the hour/minute/second of the selected city.
 *
 * The analog face must show the city's local time, so the parts come from a
 * time-zone-aware formatter instead of the `Date` accessors, which would report
 * the host machine's zone. `h23` avoids a 12/24-hour preference changing the
 * hand geometry.
 */
