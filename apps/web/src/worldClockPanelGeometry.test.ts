...
describe("world clock panel geometry", () => {
  it("reserves native caption controls and still fits below the titlebar", () => {
    const geometry = clampWorldClockPanelGeometry(
      { x: 0, y: 0, width: 900, height: 900, collapsed: false },
      { width: 800, height: 600, topInset: 64 },
    );
    expect(geometry.y).toBe(64 + WORLD_CLOCK_PANEL_MARGIN);
