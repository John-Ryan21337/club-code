/**
 * Meeting privacy is a renderer-local presentation filter. The operator opts
 * in per physical project folder, and this device then removes those folders
 * and their threads from presentation surfaces while the mode is on.
 *
 * Scope limits that the UI copy must keep truthful:
 * - This is presentation privacy, not access control. It does not change
 *   credentials, permissions, or backend authorization.
 * - Hidden projects stay connected and active provider turns continue.
 *   Local follow-up queues remain saved, but dispatch needs an open chat view.
 * - The hidden list lives only in this renderer's persisted UI state, so it
 *   never travels to the backend and never gates data delivery.
 *
 * Identities are physical project keys (environment id + normalized cwd), so
 * two environments that share a path stay distinct and a folder stays hidden
 * across restarts even when the backend mints a new project id.
 */
