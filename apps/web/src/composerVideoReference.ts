...
import { normalizeComposerTextFileName } from "./composerTextFile";
...
export function normalizeComposerVideoReferenceName(name: string): string {
  const normalized = normalizeComposerTextFileName(name);
  return normalized === "attachment.txt" ? "video" : normalized;
}
...
export function formatComposerVideoReference(analysis: ComposerVideoReferenceAnalysis): string {
  ...
  return [
    ...
    "Analyze the attached contact sheet in chronological order. If asked to recreate this as a Club Code atmosphere, build executable pointer-transparent, lifecycle-safe animation code using the existing effect/profile contracts, WebGL2 acceleration where appropriate, and the Canvas2D fallback. Do not merely embed or replay the source video unless explicitly requested.",
