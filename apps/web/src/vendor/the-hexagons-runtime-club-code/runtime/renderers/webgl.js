...
void main(){outColor=texture(uTexture,gl_FragCoord.xy/uResolution);}`;

// Volumetric shafts are intentionally smooth, low-frequency light. Rendering
// that one pass through a high-resolution intermediate surface preserves its
// appearance after linear upscale while preventing the 32-tap shader from
// executing once per 4K output pixel. Tile edges, glyphs, particles, and the
// final composition remain at the full quality-selected backing resolution.
const SHAFT_BACKING_PIXELS=Object.freeze({performance:921_600,balanced:2_073_600,cinematic:3_686_400});

export function fitShaftBackingSize(width,height,quality="cinematic"){
  const safeWidth=Math.max(1,Math.floor(width)),safeHeight=Math.max(1,Math.floor(height));
  const limit=SHAFT_BACKING_PIXELS[quality]??SHAFT_BACKING_PIXELS.cinematic;
  const scale=Math.min(1,Math.sqrt(limit/Math.max(1,safeWidth*safeHeight)));
  return{width:Math.max(1,Math.floor(safeWidth*scale)),height:Math.max(1,Math.floor(safeHeight*scale)),limit};
}
...
export function createWebGlRenderer(canvas,onStateChange=()=>{}){
  ...
  function resize(width,height,dpr,quality="cinematic"){canvas.width=Math.max(1,Math.round(width*dpr));canvas.height=Math.max(1,Math.round(height*dpr));canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;const shaftSize=fitShaftBackingSize(canvas.width,canvas.height,quality);shaftWidth=shaftSize.width;shaftHeight=shaftSize.height;configureRenderTexture(lightMaskTexture,canvas.width,canvas.height);configureRenderTexture(shaftTexture,shaftWidth,shaftHeight);gl.bindTexture(gl.TEXTURE_2D,null);gl.viewport(0,0,canvas.width,canvas.height);}
  function render(frame,settings,lights){
    ...
    if(lights.behind.enabled&&lights.behind.intensity>0){gl.bindFramebuffer(gl.FRAMEBUFFER,shaftFramebuffer);gl.viewport(0,0,shaftWidth,shaftHeight);gl.disable(gl.BLEND);gl.useProgram(programs.shaft);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,lightMaskTexture);setInt(gl,programs.shaft,"uLightMask",0);setVec2(gl,programs.shaft,"uTextureResolution",shaftWidth,shaftHeight);setVec2(gl,programs.shaft,"uLightUv",lights.behind.x/Math.max(1,frame.viewport.width),1-lights.behind.y/Math.max(1,frame.viewport.height));setVec3(gl,programs.shaft,"uShaftWhite",lights.behind.prismMode==="solid"?lights.behind.color:lights.behind.white);const shaftScale=lights.behind.type==="ripple"?.18:1;setFloat(gl,programs.shaft,"uShaftStrength",Math.min(3,Math.sqrt(lights.behind.intensity)*(.55+lights.behind.prism/12*.85))*shaftScale);setInt(gl,programs.shaft,"uShaftType",lightTypeIndex(lights.behind.type));gl.drawArrays(gl.TRIANGLES,0,3);drawCalls+=1;gl.bindFramebuffer(gl.FRAMEBUFFER,null);gl.viewport(0,0,canvas.width,canvas.height);gl.enable(gl.BLEND);gl.blendFuncSeparate(gl.ONE,gl.ONE,gl.ZERO,gl.ONE);gl.useProgram(programs.composite);gl.bindTexture(gl.TEXTURE_2D,shaftTexture);setInt(gl,programs.composite,"uTexture",0);setVec2(gl,programs.composite,"uResolution",canvas.width,canvas.height);gl.drawArrays(gl.TRIANGLES,0,3);gl.bindTexture(gl.TEXTURE_2D,null);drawCalls+=1;}
    renderedFrames+=1;if(renderedFrames===1||renderedFrames%120===0)lastError=gl.getError();return{status:lastError===gl.NO_ERROR?"rendered":"gl-error",glError:lastError,drawCalls,tileInstances:frame.grid.tiles.length,gapInstances:frame.gapInstanceCount,mesh:mode==="rhombille"?"bounded-three-prism":mode==="cairo-pentagon"?"cairo-four-pentagon":"hexagram-twelve-facet",lightShaftSamples:32,lightShaftBackingPixels:shaftWidth*shaftHeight};
  }
  return{available:true,fallbackReason:null,resize,render,forceContextLoss(){lose?.loseContext();},forceContextRestore(){lose?.restoreContext();},dispose(){if(disposed)return;disposed=true;canvas.removeEventListener("webglcontextlost",lost);canvas.removeEventListener("webglcontextrestored",restored);Object.values(programs).forEach((value)=>gl.deleteProgram(value));Object.values(buffers).forEach((value)=>gl.deleteBuffer(value));gl.deleteTexture(lightMaskTexture);gl.deleteFramebuffer(lightMaskFramebuffer);gl.deleteTexture(shaftTexture);gl.deleteFramebuffer(shaftFramebuffer);for(const mesh of[...Object.values(sideMeshes),...Object.values(topMeshes)])gl.deleteBuffer(mesh.meshBuffer);}};
}
