...
export function fitCanvasDpr(width, height, requested, settings) {
  ...
  // large viewports remain inside the advertised backing-store budget.
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const backingPixels = Math.max(1, Math.round(width * fitted)) * Math.max(1, Math.round(height * fitted));
    if (backingPixels <= limits.backingPixels) break;
    fitted *= Math.sqrt(limits.backingPixels / backingPixels) * .999;
  }
  return fitted;
}
...
function drawTiles(context,frame,settings,light){
  ...
  for(let index=0;index<frame.grid.tiles.length;index+=1){const offset=index*8,centerX=frame.tileInstances[offset],centerY=frame.tileInstances[offset+1],radius=frame.tileInstances[offset+2],height=frame.tileInstances[offset+3],phase=frame.tileInstances[offset+4],pulse=frame.tileInstances[offset+5],separation=frame.tileInstances[offset+6],pattern=frame.tileInstances[offset+7];
    ...
    for(let facet=0;facet<baseFacets.length;facet+=1){const paletteIndex=facetPaletteIndex(facet,pattern,settings),facetBase=settings.customDiamondColorsEnabled?mix(darkBase,customColors[paletteIndex],settings.foregroundIllumination):base;facetBases.push(facetBase);const groupedFacet=Math.min(2,facet),groupedHeight=mode==="rhombille"?frame.facetHeights[index*3+groupedFacet]:height;const facetHeight=groupedHeight+Math.sin(frame.time*settings.facetReliefSpeed*Math.PI*2+phase+facet*2.094)*settings.facetRelief;const centerOffset=index*6,groupCenterX=mode==="rhombille"?frame.facetCenters[centerOffset+groupedFacet]:centerX,groupCenterY=mode==="rhombille"?frame.facetCenters[centerOffset+3+groupedFacet]:centerY;const shift=tileShift(groupCenterX,groupCenterY,facetHeight,radius,frame,settings);const direction=facetDirection(mode,facet),length=Math.max(.001,Math.hypot(...direction)),sx=direction[0]/length*radius*separation*.48,sy=direction[1]/length*radius*separation*.48;
