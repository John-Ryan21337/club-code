...
export function createAtmosphereScene(
  ...
  const particles = Array.from({ length: count }, (_, index): AtmosphereParticle => {
    const matrixX = count > 0 ? ((index + 0.5) / count) * safeWidth : 0;
    ...
    if (kind === "matrix") {
      ...
      const glyphs = usesJapanese ? MATRIX_JAPANESE_GLYPHS : MATRIX_ROMAN_GLYPHS;
      ...
      const lifecycleStartY = walk ? random() * Math.max(0, safeHeight) : 0;
      const walkX =
        count > 0 ? ((index + 0.15 + random() * 0.7) / count) * safeWidth : random() * safeWidth;
      const x = walk ? walkX : matrixX;
      return {
        x,
        y: walk
          ? resolveMatrixWalkViewportY(
              lifecycleStartY,
              lifecycleProgress,
              lifecycleTravel,
              safeHeight,
          ...
          : random() * safeHeight,
        velocityX: walk
          ? resolveMatrixCenterWindVelocity(x, safeWidth, matrixCenterWindIntensity)
          : 0,
...
export function advanceAtmosphereSceneInPlace(
  ...
  const activeParticleCount = scene.particles.length;
...
export function resolveMatrixAtmosphereColorFrame(
  ...
  return {
    ...
    // Keep a requested 64x shimmer visibly fast, but distribute it across the
    // scene instead of synchronizing a large-area color transition. The seeded
    // stream phases preserve the same color distribution while its hues move,
    // rather than flashing the entire field in lockstep.
...
export function drawAtmosphereScene(
  ...
  const fallingParticleCount = scene.particles.length;
