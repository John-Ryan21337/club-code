...
export type AmbientDirectoryImageFile = File & {
 ...
 * is read only to establish order and is intentionally never returned, persisted,
 * logged, or displayed.
 */
function stableDirectoryOrder(
  ...
  const leftKey = (left.webkitRelativePath || left.name).normalize("NFC").toLowerCase();
  const rightKey = (right.webkitRelativePath || right.name).normalize("NFC").toLowerCase();
...
export function prepareAmbientImageDirectory(
  ...
  return {
    files: supported,
    skippedUnsupported: candidates.length - supported.length,
  };
}
