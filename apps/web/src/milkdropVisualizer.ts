...
export function fitMilkdropCanvas(
  ...
  // Scale to the edge limit before multiplying. Valid finite CSS dimensions
  // can otherwise overflow their product and collapse the renderer to 1x1.
  const pixelScale = Math.min(
    1,
    Math.sqrt(MILKDROP_MAX_CANVAS_PIXELS / (cssWidth * edgeDpr * (cssHeight * edgeDpr))),
  );
  ...
  const width = Math.min(MILKDROP_MAX_CANVAS_EDGE, Math.max(1, Math.floor(cssWidth * dpr)));
  return {
    width,
    height: Math.min(
      MILKDROP_MAX_CANVAS_EDGE,
      Math.floor(MILKDROP_MAX_CANVAS_PIXELS / width),
...
// The pinned CommonJS package gains a second default wrapper in Vite's lazy
// development import. Production and static imports can expose just one.
export function resolveBundledMilkdropFactory(
  module: unknown,
  ...
  for (let depth = 0; depth < 3; depth += 1) {
    ...
    if (typeof exports.createVisualizer === "function") {
      return exports.createVisualizer.bind(
        candidate,
      ) as typeof import("butterchurn").default.createVisualizer;
    }
    candidate = exports.default;
  }
  throw new Error("The bundled MilkDrop factory is unavailable.");
}
...
async function loadMilkdropRuntimeUncached(): Promise<{
  ...
  return {
    createVisualizer: resolveBundledMilkdropFactory(engineModule),
...
export class MilkdropVisualizerController {
  ...
  #requestFrame(): void {
    ...
    this.#frameHandle = this.#platform.requestAnimationFrame((timestamp) => {
      ...
      // Preset observers can stop or destroy the controller synchronously.
      // Do not render once more against a graph/context they just released.
