...
export interface ComposerThreadDraftState {
   ...
   * satisfies the `ProviderInstanceId` slug pattern, so legacy kind-keyed
...
export function deriveEffectiveComposerModelState(input: {
  ...
  const exactBaseModel = input.selectedInstanceId
    ...
    : null;
  // Never borrow another instance's inventory when an exact snapshot exists.
  // Ordinary providers may temporarily report an empty inventory during
  // startup, so retain their persisted thread/project model in that state.
  // LM Studio is different: its callable local inventory is authoritative,
  // and retaining a cloud-authored Codex model would silently misroute it.
...
function readPersistedAttachmentIdsFromStorage(threadKey: string): string[] {
  ...
  try {
    const persisted = getLocalStorageItemWithLegacy(
      COMPOSER_DRAFT_STORAGE_KEY,
      [LEGACY_COMPOSER_DRAFT_STORAGE_KEY],
