...
export function fitLocalMediaVisualizerCanvas(
  ...
  const pixelScale = Math.min(
    1,
    Math.sqrt(
      LOCAL_MEDIA_VISUALIZER_MAX_CANVAS_PIXELS / (cssWidth * edgeDpr * (cssHeight * edgeDpr)),
    ),
  );
  ...
  return {
    ...
    height: Math.min(
      LOCAL_MEDIA_VISUALIZER_MAX_CANVAS_EDGE,
      Math.floor(LOCAL_MEDIA_VISUALIZER_MAX_CANVAS_PIXELS / width),
...
export class LocalMediaAudioVisualizerController {
  ...
  #initializingContext: AudioContext | null = null;
  #graphInitialization: Promise<void> | null = null;
  ...
  #milkdropActivation: Promise<void> | null = null;
  ...
  resize(): void {
    if (this.#milkdrop !== null && !this.#spectrumRequested) {
  ...
  async sync(
    ...
    const approved = this.#isInputApproved();
    ...
    if (!this.#context && (this.#requested || this.#signalRequested)) {
      if (this.#graphInitialization !== null) await this.#graphInitialization;
      if (this.#destroyed || generation !== this.#activationGeneration) return;
      if (!this.#context) {
        const pending = this.#initializeGraph(generation);
        this.#graphInitialization = pending;
        try {
          await pending;
        } finally {
          if (this.#graphInitialization === pending) this.#graphInitialization = null;
        }
      }
    }
    if (this.#destroyed || generation !== this.#activationGeneration) return;
    ...
    // Resume can cross a source change or withdrawal of capture approval.
    // Keep the audible graph, but do not attach or restart an analysis branch.
  ...
  async destroy(): Promise<void> {
    ...
    localMediaAudioSignalStore.release(this.#signalOwner);
    clearCanvas(this.#canvas);
    const contexts = new Set([this.#context, this.#initializingContext]);
    this.#context = null;
  ...
  async #syncMilkdrop(settings: LocalMediaVisualizerSettings, generation: number): Promise<void> {
    // Import/initialization owns this exact canvas. Keep its stopped renderer
    // for newer settings: losing a WebGL context would make this canvas
    // unusable for the next style change. Owner teardown releases it.
    if (this.#milkdropActivation !== null) await this.#milkdropActivation;
    ...
    if (
      ...
      !this.#isInputApproved()
    )
      return;

    if (this.#milkdrop === null) {
      const pending = this.#createMilkdrop(settings, generation, context, source);
      this.#milkdropActivation = pending;
      try {
        await pending;
      } finally {
        if (this.#milkdropActivation === pending) this.#milkdropActivation = null;
      }
    }
    if (
      this.#destroyed ||
  ...
  async #createMilkdrop(
    settings: LocalMediaVisualizerSettings,
    generation: number,
    ...
    if (!activation.ok) {
      if (generation !== this.#activationGeneration || !this.#requested || !this.#isInputApproved())
        return;
      this.#publishMilkdropState({
        status: "error",
        presetNames: [],
  ...
  async #initializeGraph(generation: number): Promise<void> {
    ...
    try {
      context = this.#platform.createAudioContext();
    } catch {
      this.#reportGraphFailure(generation);
      return;
    }
    // Resume may wait for a future user gesture. Own this context immediately
    // so unmount can close it without waiting for that promise to settle.
    ...
    try {
      ...
      if (
        ...
        if (context.state !== "running") this.#reportGraphFailure(generation);
      ...
      try {
        ...
        // Allocate analysis before taking ownership of the element audio path.
  ...
  #reportGraphFailure(generation: number): void {
    if (this.#destroyed || generation !== this.#activationGeneration) return;
    this.#publishMilkdropState({
      status: "error",
      presetNames: [],
