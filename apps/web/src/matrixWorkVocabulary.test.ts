...
describe("Matrix live work vocabulary", () => {
  ...
  it("shares a traversal budget across the whole activity tail", () => {
    const wide = {
      branches: Array.from({ length: 32 }, () => ({
        nested: Array.from({ length: 32 }, () => ({})),
      })),
    };
    const entries = [
      activity("1", { itemType: "file_change", data: { path: "Visible.ts" } }),
      ...Array.from({ length: 24 }, (_, index) =>
        activity(String(index + 2), { itemType: "dynamic_tool_call", data: wide }),
      ),
      activity("99", { itemType: "file_change", data: { path: "PastBudget.ts" } }),
    ];
    const result = deriveMatrixWorkVocabulary(entries);
    expect(result.english).toContain("Visible.ts");
    expect(result.japanese).toContain("Visible.ts");
    expect(result.english).not.toContain("PastBudget.ts");
    expect(result.japanese).not.toContain("PastBudget.ts");
  });

  it("does not infer operation labels from fragments of unrelated words", () => {
    const result = deriveMatrixWorkVocabulary([
      activity("1", {
        observed: {
          providerObserved: true,
          operation: "preview repairman testing archetypes formatting builder planets contextless",
        },
      }),
    ]);
  ...
  it("ignores accessor-backed activity and classification fields", () => {
    const getter = () => {
    ...
    const observed = { providerObserved: true };
    Object.defineProperty(observed, "operation", { enumerable: true, get: getter });
    const payload = { itemType: "file_change", observed };
    Object.defineProperty(payload, "data", { enumerable: true, get: getter });
    expect(deriveMatrixWorkVocabulary([entry, activity("2", payload)])).toEqual({
      english: ["WRITE"],
      japanese: ["書込"],
    });
  });

  it("bounds encoded inputs and traverses cyclic structured data without invoking getters", () => {
    const getter = () => {
      throw new Error("must not read accessors");
    };
    const data: Record<string, unknown> = { path: "safe.ts" };
    data.self = data;
    Object.defineProperty(data, "filePath", { enumerable: true, get: getter });
    const result = deriveMatrixWorkVocabulary([activity("1", { itemType: "file_change", data })]);
    expect(result.english).toContain("safe.ts");
    expect(decodeMatrixWorkVocabulary(" ".repeat(8_193))).toEqual({ english: [], japanese: [] });
    const wide = {
      branches: Array.from({ length: 10_000 }, (_, index) => ({ path: `file${index}.ts` })),
    };
    const bounded = deriveMatrixWorkVocabulary([
      activity("2", { itemType: "file_change", data: wide }),
    ]);
    expect(bounded.english.length).toBeLessThanOrEqual(32);
    expect(bounded.english).not.toContain("file9999.ts");
  });
