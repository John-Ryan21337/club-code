...
import { setEmbeddedBrowserChatWidth } from "../embeddedBrowserChatLayout";
...
export function useEmbeddedBrowserLayout(
  ...
  useEffect(() => {
    ...
    setEmbeddedBrowserChatWidth(active ? (stacked ? size.width : splitPosition - 4) : null);
    root.setProperty("--cafe-browser-chat-contain", active ? "layout" : "none");
    root.setProperty("--cafe-browser-chat-min-width", active ? "0px" : "auto");
    root.setProperty(
      "--cafe-browser-chat-width",
      active && !stacked ? `${splitPosition - 4}px` : "100%",
    );
    root.setProperty(
      "--cafe-browser-chat-height",
      `${active && stacked ? splitPosition - 4 : height}px`,
    );
    return () => {
      root.removeProperty("--cafe-browser-chat-width");
      root.removeProperty("--cafe-browser-chat-height");
      root.removeProperty("--cafe-browser-chat-contain");
      root.removeProperty("--cafe-browser-chat-min-width");
      setEmbeddedBrowserChatWidth(null);
    };
  }, [height, mode, size.width, splitPosition, stacked, visible]);
