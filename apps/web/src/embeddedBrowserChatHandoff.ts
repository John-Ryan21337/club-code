...
// Submitted queue entries may survive a renderer reload. Recover their browser
// suffix into memory when the user edits or retries them; keep it out of drafts.
export function recoverEphemeralBrowserDraft(
  prompt: string,
  contextStart?: number,
): {
  ...
  const marker = "[User-approved embedded browser snapshot]";
  const start =
    contextStart !== undefined &&
    Number.isSafeInteger(contextStart) &&
    contextStart >= 0 &&
    contextStart <= prompt.length
      ? contextStart
      : prompt.indexOf(marker);
  if (start < 0 || (contextStart === undefined && start > 0 && prompt[start - 1] !== "\n")) {
    return { persistedDraftPrompt: prompt, ephemeralBrowserContext: "" };
  }
  return {
    persistedDraftPrompt: prompt.slice(0, start).trimEnd(),
    ephemeralBrowserContext: prompt.slice(start),
  };
}
...
export function buildEphemeralBrowserDispatchPrompt(
  ...
  if (!contextForSend) {
    return { dispatchPrompt: persistedDraftPrompt, persistedDraftPrompt, ephemeralBrowserContext };
  }
  const separator =
    persistedDraftPrompt.trim().length > 0 && contextForSend.length > 0 ? "\n\n" : "";
  const dispatchPrompt = `${persistedDraftPrompt.trimEnd()}${separator}${contextForSend}`;
  if (dispatchPrompt.length > PROVIDER_SEND_TURN_MAX_INPUT_CHARS) {
    throw new Error("Shorten the draft or browser context before sending.");
  }
  return {
    dispatchPrompt,
    ...
    ...(contextForSend
      ? {
          browserContextStart: persistedDraftPrompt.trimEnd().length + separator.length,
        }
      : {}),
  };
}
