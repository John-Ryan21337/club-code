...
describe("meeting privacy presentation filtering", () => {
  ...
  it("hides a thread whose project identity is not resolved yet", () => {
    // Fail closed: while the mode is on, a thread that points at a project the
    // renderer has not received cannot be proved safe, so it stays out of the
    // list until the project arrives.
    const hiddenProjectKeys = [meetingPrivacyProjectKey(projects[1])];
    const threads = [
      {
        id: "thread-unresolved",
        environmentId: "environment-local",
        projectId: "project-not-loaded-yet",
    ...
    // With the mode off the same thread is returned untouched.
    expect(
      filterThreadsForMeetingPrivacy(threads, projects, {
        enabled: false,
        hiddenProjectKeys,
  ...
  it("treats an empty hidden list as a no-op so route guards stay open", () => {
    expect(
      resolveMeetingPrivacyRouteDisposition({
        enabled: true,
  ...
  it("bounds and sanitizes persisted identities without accepting control characters", () => {
    ...
    expect(result).not.toContain("environment-local:/nul\0byte");
  });
});
