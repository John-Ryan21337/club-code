...
/**
 * Optional current-weather reader for the world clock panel.
 *
 * Security and privacy boundary:
 * - The endpoint is a fixed compile-time HTTPS constant. Nothing in a provider
 *   payload, a setting, or a URL can redirect this request.
 * - The request carries no credentials, no referrer, and no API key, and the
 *   only outbound data is the fixed coordinates of the cities the user picked.
 * - Every response is size-bounded, content-type checked, and parsed field by
 *   field before it reaches the UI. Error text is reduced to a fixed category
 *   string, so a hostile or broken endpoint cannot place its own text on screen.
 * - A failure starts a cooldown, so a broken endpoint cannot become a retry loop.
 *
 * The caller (the widget) is responsible for the consent gate: this module is
 * never reached unless the local renderer consent is on. See
 * `./worldClockWeatherConsent`.
 */
