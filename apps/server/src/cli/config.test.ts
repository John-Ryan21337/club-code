...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  cafeCodeHome: "/tmp/t3-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("enables HTTPS and resolves explicit HTTPS port flags", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "t3-cli-config-https-flags-base");

      const resolved = yield* resolveServerConfig(
        {
          mode: Option.some("web"),
          port: Option.some(8788),
          httpsPort: Option.some(9443),
          host: Option.some("127.0.0.1"),
          baseDir: Option.some(baseDir),
          cwd: Option.none(),
          devUrl: Option.none(),
      ...
      expect(resolved.httpsEnabled).toBe(true);
      expect(resolved.httpsPort).toBe(9443);
    }),
  );
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  CAFE_CODE_BOOTSTRAP_FD: String(fd),
                  CAFE_CODE_NO_BROWSER: "true",
                  CAFE_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  CAFE_CODE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
