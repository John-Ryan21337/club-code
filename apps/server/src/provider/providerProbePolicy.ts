...
export const deterministicProviderProbePhaseOffsetMs = (
  ...
  // `Math.max` propagates NaN, which would turn the modulo below into a NaN
  // delay. Callers derive the interval from configuration, so treat any
  // non-finite value as the smallest legal interval rather than trusting it.
  const boundedIntervalMs = Number.isFinite(intervalMs) ? Math.max(1, Math.floor(intervalMs)) : 1;
  let hash = 0x811c9dc5;
 ...
 * Without this reconciliation, repeatedly restarting Club Code could prevent the
