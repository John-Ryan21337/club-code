...
/** Copies only the provider child's environment; sibling sessions keep their own credentials. */
export function withAgentBrowserAuthorization(
  environment: NodeJS.ProcessEnv,
  config: AgentBrowserMcpConfig | undefined,
  platform: NodeJS.Platform = process.platform,
): NodeJS.ProcessEnv {
  if (!config) return environment;
  const child = { ...environment };
  const key = "CAFE_CODE_AGENT_BROWSER_MCP_AUTHORIZATION";
  if (platform === "win32") {
    for (const existing of Object.keys(child)) {
      if (existing.toUpperCase() === key) delete child[existing];
    }
  }
  child[key] = config.authorization;
  return child;
}
...
export class AgentBrowserBridge {
  ...
  #closed = false;
  #rendererTimer: ReturnType<typeof setTimeout> | undefined;
  #activeHttpRequests = 0;

  async start(): Promise<void> {
    if (this.#closed) throw new Error("The browser bridge has closed.");
  ...
  async close(): Promise<void> {
    ...
    if (server) {
      // Shutdown must not wait for a provider that stopped midway through an
      // HTTP body. Every browser action has already been rejected above.
  ...
  poll(context: AgentBrowserSessionContext): AgentBrowserPollResult {
    // A delayed heartbeat cannot restore requests from an expired page lease.
    this.#expireIfNeeded();
    if (context.defaultAccess) {
      ...
      this.#rendererTimer = setTimeout(() => {
        this.#revokeInternal("The Agent Browser disconnected.");
      }, 5_000);
  ...
  complete(input: AgentBrowserCompleteInput): AgentBrowserCompleteResult {
    ...
    if (Date.now() >= Date.parse(pending.request.expiresAt)) {
      this.#rejectPending(pending, "The browser action expired before completion.");
      return { accepted: false, grant: this.#state() };
    }
    if (
  ...
  async mcpConfig(identity: AgentIdentity): Promise<AgentBrowserMcpConfig> {
    await this.start();
    if (this.#closed) throw new Error("The browser bridge has closed.");
    // A replacement process must not inherit the retired process's capability,
    // even when its Cafe thread and configured provider instance stay the same.
    const key = identityKey(identity);
    if (this.#credentialsByIdentity.has(key)) {
      this.revokeForIdentity(identity, "The provider session was replaced.");
    }
  ...
  releaseMcpConfig(config: AgentBrowserMcpConfig): void {
    // A delayed finalizer from an older process cannot revoke its replacement.
    if (this.#credentialsByIdentity.get(identityKey(config)) === config.authorization) {
      this.revokeForIdentity(config, "The provider session ended.");
    }
  }
  ...
  async enqueue(
    ...
    const rendererDisconnected = this.#automaticContext && Date.now() - this.#lastPollAt >= 5_000;
    this.#expireIfNeeded();
    if (rendererDisconnected) {
      throw new Error("The Agent Browser is disconnected. Open it in Cafe Code.");
    }
    ...
    if (!grant)
      throw new Error(
        "Open Agent Browser in Cafe Code and share the current page. Thread access is on by default.",
      );
    if (
  ...
  #expireIfNeeded(): void {
    if (this.#automaticContext && Date.now() - this.#lastPollAt >= 5_000) {
      this.#revokeInternal("The Agent Browser disconnected.");
      return;
    }
    if (
  ...
  async #handleHttp(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    // This endpoint is for provider processes, never browser-origin fetches.
    if (request.headers.origin !== undefined) return fail(403, "Forbidden.");
    if (request.method !== "POST" || request.url !== "/mcp")
      return fail(405, "Method not allowed.");
    const host = this.#url ? new URL(this.#url).host : undefined;
    ...
    // Initialization and tool-list requests do not enter the action queue.
    // Give those requests their own process-wide concurrency and time bounds.
    if (this.#activeHttpRequests >= 64) return fail(429, "Too many requests.");
    this.#activeHttpRequests += 1;
    const deadline = setTimeout(() => response.destroy(), AGENT_BROWSER_REQUEST_TIMEOUT_MS + 5_000);
  ...
  #makeMcpServer(identity: AgentIdentity, authorization: string): McpServer {
    const server = new McpServer({ name: "cafe-code-browser", version: "1.0.0" });
