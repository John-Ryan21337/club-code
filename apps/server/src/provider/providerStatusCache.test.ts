...
it.layer(NodeServices.layer)("providerStatusCache", (it) => {
  ...
  it("drops a retired probe-diagnostics cache shape when the provider no longer reports one", () => {
    const cachedCodex = makeProvider(CODEX_DRIVER, {
      probeDiagnostics: {
        attemptCount: 2,
        consecutiveInconclusiveCount: 2,
    ...
    const hydrated = hydrateCachedProvider({
      cachedProvider: cachedCodex,
      fallbackProvider: makeProvider(CODEX_DRIVER),
    });

    assert.isUndefined(hydrated.probeDiagnostics);
  });
