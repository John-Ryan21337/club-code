...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it("keeps Codex OSS and cloud model catalogs isolated across an inconclusive probe", () => {
        // Club Code isolates LM Studio's authoritative `/v1/models` inventory
        // from the static cloud catalog. Inconclusive retention republishes the
        // previous auth identity, so the isolation rule must keep reading the
        // raw incoming snapshot or a cloud rebuild would contaminate the local
        // model list.
        const localProvider = {
          instanceId: ProviderInstanceId.make("codex"),
          driver: ProviderDriverKind.make("codex"),
        ...
        const inconclusiveCloudProvider = {
          ...localProvider,
          status: "warning",
          auth: { status: "unknown" },
          checkedAt: "2026-04-14T00:05:00.000Z",
        ...
        const merged = mergeProviderSnapshot(localProvider, inconclusiveCloudProvider);
