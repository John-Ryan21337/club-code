  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "cafe-code-web",
          attributes: {
            "service.runtime": "cafe-code-web",
            ...
            read: (input) =>
              Effect.succeed({
                ...
                network: {
                  ...
                  detail: "Network throughput telemetry is unavailable.",
                },
                gpu: {
                  status: "unavailable",
                  adapters: [],
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/cafe-code/environment");
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "cafe-code-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(response.network.receiveBytesPerSecond, 1_500);
      }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("records setup-script failures without aborting bootstrap turn start", () =>
    Effect.gen(function* () {
      ...
      const runForThread = vi.fn(
        (_: Parameters<ProjectSetupScriptRunnerShape["runForThread"]>[0]) =>
          Effect.fail(new ProjectSetupScriptRunnerError({ message: "setup unavailable" })),
      );
