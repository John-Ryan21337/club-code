...
class BinaryReader {
  ...
  string(): string {
    ...
    if (raw[raw.length - 1] !== 0 || raw.subarray(0, -1).includes(0)) {
      throw new Error("OpenRGB returned an invalid terminated string.");
    }
    return new TextDecoder("utf-8", { fatal: true }).decode(raw.subarray(0, -1));
  }
  ...
  finish(): void {
    if (this.offset !== this.buffer.length)
      throw new Error("OpenRGB returned unexpected trailing data.");
  }
}

function encodeString(value: string): Buffer {
  const body = Buffer.from(value, "utf8");
  if (body.length >= 4_096) throw new Error("OpenRGB output string exceeds the safety limit.");
...
function parseControllerData(deviceId: number, payload: Buffer): OpenRgbControllerRecord {
  ...
  const supported =
    colorCount > 0 &&
    colorCount === ledCount &&
    modes.some((mode) => (mode.flags & MODE_FLAG_HAS_PER_LED_COLOR) !== 0);
  return {
    deviceId,
...
export class OpenRgbConnection {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const closed = () => finish(new Error("OpenRGB closed before connection completed."));
      const timer = setTimeout(() => {
        const error = new Error("OpenRGB connection exceeded its deadline.");
        finish(error);
      ...
      this.socket.connect(this.port, OPENRGB_HOST, () => finish());
    });
  }
  ...
  private onData(data: Buffer): void {
    if (data.length + this.buffer.length > MAX_PACKET_BYTES + 16) {
      this.socket.destroy(new Error("OpenRGB receive buffer exceeds the safety limit."));
      return;
    }
  ...
  send(deviceId: number, packetId: number, payload: Uint8Array = Buffer.alloc(0)): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onClose = () => finish(new Error("OpenRGB closed before the write completed."));
      const onError = (error: Error) => finish(error);
      const timer = setTimeout(() => {
        const error = new Error("OpenRGB write exceeded its deadline.");
        finish(error);
        this.socket.destroy(error);
      }, this.timeoutMs);
      timer.unref?.();
      ...
      try {
        this.socket.write(Buffer.concat([header, payload]), (error) => finish(error ?? undefined));
      } catch (cause) {
        finish(cause instanceof Error ? cause : new Error("OpenRGB write failed."));
      }
    });
  }
  ...
  async request(
    ...
    const timer = setTimeout(() => {
      const error = new Error("OpenRGB response exceeded its deadline.");
    ...
    try {
      // Attach both rejection handlers before either operation can settle.
      [, packet] = await Promise.all([this.send(deviceId, packetId, payload), response]);
    } catch (cause) {
      const error = cause instanceof Error ? cause : new Error("OpenRGB request failed.");
      this.rejectPending(error);
      this.socket.destroy(error);
      throw error;
    } finally {
...
export class OpenRgbHardwareLightingAdapter implements HardwareLightingAdapter {
  ...
  private connecting: OpenRgbConnection | null = null;
  ...
  /** Retire owned network work when the service's whole-operation deadline expires. */
  abort(): void {
    this.disconnect();
  }
  ...
  private async ensureConnected(force = false): Promise<void> {
    ...
    try {
      ...
      // The parser below implements the documented version-5 layout only.
      if (protocolVersion !== OPENRGB_PROTOCOL_VERSION) {
        throw new Error("OpenRGB protocol version 5 is required.");
      }
      await connection.send(0, PACKET.setClientName, Buffer.from("Cafe Code\0", "utf8"));
      ...
      if (new Set(controllers.map((controller) => controller.id)).size !== controllers.length) {
        throw new Error("OpenRGB returned ambiguous controller identities.");
      }
      ...
      this.detail =
        error instanceof Error && error.message.includes("ECONNREFUSED")
          ...
          : "Cafe Code could not complete the bounded OpenRGB loopback capability check.";
      throw error;
    } finally {
