    ...
    search: ({ query, maxResults, clientId }) =>
      Effect.gen(function* () {
        ...
        yield* Effect.acquireRelease(
          Effect.suspend(() =>
            activeRequests >= 4
              ? Effect.fail(error("rate-limited"))
              : Effect.sync(() => {
                  activeRequests += 1;
                }),
          ),
        ...
        const { response, body } = yield* Effect.tryPromise({
          try: async (signal) => {
            ...
            return { response, body: await readBoundedJson(response, signal) };
          },
          catch: () => error("unavailable"),
        }).pipe(
          Effect.timeoutOrElse({
            duration: UPSTREAM_TIMEOUT_MS,
            orElse: () => Effect.fail(error("unavailable")),
          }),
        );
...
export const YouTubePublicDiscoveryLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return makeYouTubePublicDiscovery({
      apiKey:
        config.youtubePublicDiscoveryEnabled && config.youtubePublicDiscoveryApiKey
          ? Redacted.value(config.youtubePublicDiscoveryApiKey)
          : undefined,
    });
  }),
);
