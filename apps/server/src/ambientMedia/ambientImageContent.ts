...
export const AMBIENT_IMAGE_ID_PATTERN = /^sha256-[a-f0-9]{64}\.(?:gif|jpe?g|png|webp)$/;
const MAX_GIF_FRAMES = 240;
const MAX_GIF_CUMULATIVE_PIXELS = 64_000_000;
const MAX_GIF_DURATION_CENTISECONDS = 12_000;

export const AMBIENT_IMAGE_MIME_BY_EXTENSION = {
  ".gif": "image/gif",
  ".jpg": "image/jpeg",
  ".jpeg": "image/jpeg",
  ".png": "image/png",
...
export const AMBIENT_IMAGE_EXTENSION_BY_MIME = {
  "image/gif": ".gif",
  "image/jpeg": ".jpg",
  "image/png": ".png",
  "image/webp": ".webp",
...
export interface AmbientImageHeader {
  readonly mimeType: AmbientImageMimeType;
  readonly width: number;
  readonly height: number;
}

export interface AmbientImageContentValidationFailure {
  readonly ok: false;
  readonly error: {
    readonly code: "invalid-image" | "too-large" | "unsupported-type";
    readonly message: string;
    readonly status: 400 | 413 | 415;
  };
}

export interface AmbientImageContentValidationSuccess {
  readonly ok: true;
  readonly header: AmbientImageHeader;
}

export type AmbientImageContentValidationResult =
  | AmbientImageContentValidationFailure
  | AmbientImageContentValidationSuccess;
...
function parseGif(bytes: Uint8Array): AmbientImageHeader | null {
...
function parsePng(bytes: Uint8Array): AmbientImageHeader | null {
...
function parseJpeg(bytes: Uint8Array): AmbientImageHeader | null {
  ...
  for (let offset = 2; offset + 4 <= bytes.byteLength; ) {
    ...
    if (marker === 0xda) {
      ...
      for (let scanOffset = offset + length; scanOffset + 1 < bytes.byteLength; scanOffset++) {
        ...
        if (bytes[next] === 0xd9)
          return next + 1 === bytes.byteLength
            ? { mimeType: "image/jpeg", width: dimensions.width, height: dimensions.height }
            : null;
        scanOffset = next;
      }
      return null;
...
function parseWebp(bytes: Uint8Array): AmbientImageHeader | null {
  ...
  if (!imageDimensions || !sawImageData || offset !== bytes.byteLength) return null;
  if (
    canvasDimensions &&
...
export function parseAmbientImageHeader(bytes: Uint8Array): AmbientImageHeader | null {
  return parsePng(bytes) ?? parseGif(bytes) ?? parseJpeg(bytes) ?? parseWebp(bytes);
}

export function normalizeAmbientImageMime(value: string | undefined): AmbientImageMimeType | null {
...
export function validateAmbientImageContent(input: {
  readonly bytes: Uint8Array;
  readonly declaredMimeType?: string;
}): AmbientImageContentValidationResult {
  ...
  const declared = normalizeAmbientImageMime(input.declaredMimeType);
  ...
  const header = parseAmbientImageHeader(input.bytes);
  if (!header) {
    return {
      ok: false,
      error: {
