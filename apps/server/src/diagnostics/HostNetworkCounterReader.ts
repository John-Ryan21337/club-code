...
export function parseWindowsNetworkCounterOutput(output: string): HostNetworkCounters | null {
  if (Buffer.byteLength(output, "utf8") > MAX_WINDOWS_OUTPUT_BYTES) return null;
...
function makeReader(
  ...
  let activeLinuxRead: {
    readonly result: Promise<HostNetworkCounters | null>;
    readonly finish: (value: HostNetworkCounters | null) => void;
  } | null = null;

  const readLinux = (): Promise<HostNetworkCounters | null> => {
    if (activeLinuxRead !== null) return activeLinuxRead.result;
    let resolveResult!: (value: HostNetworkCounters | null) => void;
    const result = new Promise<HostNetworkCounters | null>((resolve) => {
    ...
    const record = {
      result,
      finish: (value: HostNetworkCounters | null) => {
        if (settled) return;
        settled = true;
    ...
    activeLinuxRead = record;
    // A timed-out kernel read retains the one slot until completion. Repeated
    // project requests cannot accumulate in-process filesystem operations.
    deadline = scheduleDeadline(() => record.finish(null), timeoutMs + forceSettleGraceMs);
    void Promise.resolve()
      .then(() => readTextFile("/proc/net/dev", "utf8"))
      .then(
        (source) => {
          if (!settled) record.finish(parseLinuxNetworkCounterSource(source));
        },
        () => record.finish(null),
      )
      .finally(() => {
        if (activeLinuxRead === record) activeLinuxRead = null;
      });
    return result;
  };
