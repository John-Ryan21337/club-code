...
describe("HostNetworkCounterReader", () => {
  it("bounds a stuck Linux read and retains its one slot until the read completes", async () => {
    vi.useFakeTimers();
    ...
    expect(readTextFile).toHaveBeenCalledExactlyOnceWith("/proc/net/dev", "utf8");
    await vi.advanceTimersByTimeAsync(150);
    await expect(first).resolves.toBeNull();
    await expect(reader.read()).resolves.toBeNull();
    expect(readTextFile).toHaveBeenCalledTimes(1);
    resolveRead("eth0: 100 1 0 0 0 0 0 0 200 1 0 0 0 0 0 0");
  ...
  it("returns only aggregate counters from the live Linux path and sanitizes read failures", async () => {
    const readTextFile = vi
      .fn()
      .mockResolvedValueOnce("private-interface: 100 1 0 0 0 0 0 0 200 1 0 0 0 0 0 0")
      .mockRejectedValue(new Error("private path"));
    const reader = makeHostNetworkCounterReaderForTest({ platform: "linux", readTextFile });
    await expect(reader.read()).resolves.toEqual({ receivedBytes: 100, transmittedBytes: 200 });
    await Promise.resolve();
    await expect(reader.read()).resolves.toBeNull();
    await reader.close();
  });
  ...
  it("rejects malformed, oversized, and unsafe counter output", () => {
    ...
    expect(parseWindowsNetworkCounterOutput("0".repeat(257) + ",1")).toBeNull();
