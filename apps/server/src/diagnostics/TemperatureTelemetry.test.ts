...
describe("TemperatureTelemetry", () => {
  it("classifies supported Linux drivers when optional channel labels are absent", () => {
    const result = temperatureTelemetryFromRawSamples([
      { source: "linux-hwmon", name: "amdgpu edge", identifier: "/amdgpu/temp1", value: 51 },
      { source: "linux-hwmon", name: "coretemp temp1", identifier: "/coretemp/temp1", value: 49 },
      { source: "linux-hwmon", name: "k10temp temp1", identifier: "/k10temp/temp1", value: 47 },
      {
        source: "linux-hwmon",
        name: "amdgpu Memory Junction",
        identifier: "/amdgpu/temp3",
        value: 55,
      },
    ]);
    expect(result.sensors.map((sensor) => sensor.kind)).toEqual(["gpu", "cpu", "cpu", "vram"]);
  });
