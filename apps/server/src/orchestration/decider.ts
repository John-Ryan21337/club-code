  ...
  switch (command.type) {
    ...
    case "thread.manual-follow-up.activate": {
      ...
      if (
        ...
        return yield* rejectManualFollowUpCommand(
          command,
          `Thread '${command.threadId}' is not authoritatively ready for automatic manual follow-up activation. The queued follow-up remains pending until the provider is ready or the operator explicitly sends it.`,
        );
      }
