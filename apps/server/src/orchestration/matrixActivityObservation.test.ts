...
it("rejects quoted command tokens and padded values beyond their input bounds", () => {
  const longPath = "/" + "x".repeat(512) + "/curl";
  for (const command of [
    '"' + longPath + '" https://example.test',
    " ".repeat(4096) + "curl",
    "curl" + " ".repeat(4096),
  ]) {
    expect(
      classifyMatrixActivityObservation({ itemType: "command_execution", data: { command } }),
    ).toBeUndefined();
  }
  expect(
    classifyMatrixActivityObservation({
      itemType: "dynamic_tool_call",
      data: { toolName: " ".repeat(128) + "web_fetch" },
    }),
  ).toBeUndefined();
  expect(
    classifyMatrixActivityObservation({
      itemType: "command_execution",
      data: { command: '"/short/path/curl" https://example.test' },
    }),
  ).toEqual(observation("network"));
});
